...
Three stages, each independently re-runnable:
    ...
    python -m poe_repair.experiments.residual_between_mono_and_poe.delta_structure

    # Collect only (samples PoE trajectories for each seed; idempotent):
    python -m poe_repair.experiments.residual_between_mono_and_poe.delta_structure --stage collect

    # Re-run prongs and figure from cached tensors.pt:
    python -m poe_repair.experiments.residual_between_mono_and_poe.delta_structure --stage analyze

    # Push to N=8 (e.g. if Prong B was marginal):
    python -m poe_repair.experiments.residual_between_mono_and_poe.delta_structure --seeds 42,43,44,45,46,47,48,49

    # CFG sanity (single seed, unguided ε-space):
    python -m poe_repair.experiments.residual_between_mono_and_poe.delta_structure --cfg-sanity --seeds 42
