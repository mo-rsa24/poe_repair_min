...
[figure-01-the-seven-paper-figures.md](../../plans/03-does-the-correction-cause-composition/plans/figures/08-the-seven-paper-figures.md)
...
## F1: where each reading of "a cat and a dog" lands

**The question.** What does the failure actually look like, and where does PoE put the
picture instead?
...
only place the paper shows that, and it does it with pictures rather than a rate.

**What is plotted.** A drawn surface, where height is how readily the model produces
an image at that point of an illustrative plane. Three peaks, each with the real
render pinned to it by a leader: "a cat" alone, "a dog" alone, and the two-animal
scene "a cat and a dog" reaches, which is its own peak away from both. A fourth mark,
an X, sits on the saddle between the two single-animal peaks, with the PoE render
hanging off it. A dashed path runs from that X to the two-animal peak.

**What to look at.** The X and the dashed path. The product of two Gaussians peaks at
their midpoint, so the X is drawn where the maths puts it, and the picture hanging
there is one fused animal. The dashed path is the distance that maximising the
product never covers.

**What it may not claim.** Anything about how often this happens: it shows one cell.
And nothing about the surface's geometry is a measurement, so the plane is not an
embedding, no distance on it is a real distance, and the three peaks are drawn at
equal height on purpose, because the figure makes no claim about relative mass.

**The numbers.** None on the figure, by design. The caption carries the cell's
provenance instead, since the figure face carries no chip.

**Built.** `python scripts/where_poe_lands_on_the_manifold.py --layout print --seeds 12`
→ `paper/iclr/figures/where-the-product-lands-on-the-manifold-seed12-print.pdf`.
The four renders come from one cell in
`outputs/showcase/where_each_condition_lands/pairs/a_cat__x__a_dog`: seed 12, one
starting latent, 50 steps, guidance 7.5, `adapter_attached_before_references: false`
in the manifest beside them, so the only thing separating the four pictures is what
the sampler was conditioned on. The script prints all four paths on every run.

Seed 12 is the cell to use. The joint reference renders two dogs on seeds 10 and 13
and is ambiguous on 14 and 15, so those cells cannot carry this figure; seed 9 is
correct but renders the cat as an illustration beside three photographs, which reads
as meaning and is not. Seeds 11 and 16 also work.

The surface is drawn by the script rather than generated by an image model, for two
reasons found the hard way. An image model's own lettering measured 2.4pt once the
panel sat in the ICLR column. And a generated fused animal sitting beside a real one
is exactly the confusion the figure exists to prevent.

**The `--layout` switch.** `print` is the version the paper uses: at a 5.5in column
the four renders are what carry the argument, and at the `spacious` default's 0.95in
they print at 142px, where the fused animal's dog muzzle on a cat's frame stops being
visible. `print` grows every thumbnail to 1.26in, trims the plane's empty skirt, and
coarsens the mesh, whose fine white lines on a coloured ground are where print moire
shows up.
...
**Built.** `python scripts/compose_rate_vs_correction.py` → `paper/iclr/figures/compose-rate-as-correction-rises.pdf`.
Cell is cat × dog seed 9. `compose-rate-as-correction-rises-for-a-dissimilar-pair.pdf` is elephant × penguin, held in
...
**Built.** `python scripts/correction_size_vs_run_position.py` → `paper/iclr/figures/correction-size-over-the-denoising-run-across-17-pairs.pdf`.
