...
so every include is a bare filename: `\includegraphics[width=\textwidth]{F2-dose-response}`.
...
```
F2-dose-response.pdf              the paper figure for slot F2
F2-dose-response-halfwidth.pdf    the same result, laid out for a talk or poster
F2b-dissimilar-pair.pdf           a second cell for the same slot, answering one objection
```
...
```bash
python scripts/make_f2.py --layout grid5 --seed 9    # F2-dose-response
python scripts/make_f2.py --layout shared0 --seed 9  # F2-dose-response-halfwidth
python scripts/make_f2.py --layout grid5 --pair an_elephant__x__a_penguin --seed 10 \
       --name F2b-dissimilar-pair
```
