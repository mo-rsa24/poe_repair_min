...
| F2 | **built** | the correction is what is missing: more of it composes more, and two size-matched fakes stay flat | 3x5 grid of real cells (row per injected vector, column per strength) above the three curves, shared λ axis, controls gray | [review](../../plans/closing-the-compositional-gap/plans/does-the-correction-cause-composition/review/hypothesis-02-more-correction-more-composition.md), bar ✅ and both open questions ✅. 3% to 94% for the real correction, controls at or below 6%, AUC 0.387 against 0.023 and 0.039, 32 cells per row-and-strength, size floor MIN_BOX_FRACTION=0.25 chosen from real detections. **The caption may quote these numbers.** | `dose_curves.png` plus `dose_strip_an_elephant__x__a_penguin_seed10.png` (3x5 grid) |
