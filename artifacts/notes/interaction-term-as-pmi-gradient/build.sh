#!/usr/bin/env bash
# Render the captured pressure tests as self-contained web pages with real maths.
set -euo pipefail
cd "$(dirname "$0")"
...
# One page per capture: these are independent documents, not rungs of one ladder.
for f in *.md; do
  pandoc "$f" "${OPTS[@]}" \
    --toc --standalone --css tufte.css \
    -o "${f%.md}.html"
  echo "wrote ${f%.md}.html"
done
