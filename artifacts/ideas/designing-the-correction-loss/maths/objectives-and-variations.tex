...
\B{The guidance weight is inside the loss, not outside it.} Both sides carry the guidance weight. Setting $w=1$ in
...
\B{The composition rule is blind to one move, and the loss at guidance 7.5 is not.}
...
\begin{equation}
  ...
  \label{eq:gauge}
\end{equation}
The at guidance 1 composition does not move at all. The guided one moves by $-(w-1)v = -6.5v$, and
so does the loss at guidance 7.5, because its target is built from the cached $\bar u$ and does not follow.

That asymmetry is the problem, and it is worse than blindness. \textbf{The loss at guidance 7.5 can lower
...
\item $a+b-u$ is unchanged by adding the same $v$ to $a$ and to $u$; $T^{(w)}$ changes by
  $-(w-1)v$. So the loss at guidance 1es cannot see that move and the one at guidance 7.5s can \emph{use} it,
