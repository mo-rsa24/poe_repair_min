...
failed-corrector controls 05–06, now archived under `artifacts/plans/archived/phases/`.
...
## Tasks
- [x] ✅ Groundwork: veracity (gap reachable), residual diagnostics (target structured), CFG-mask floor. → G01/G02/G03 landed (`artifacts/plans/archived/phases/01-03`).
...
- [x] ✅ Negative controls: group-A external correctors (latent CNN/UNet/frozen-MLP) + internal-force (attention-overlap, score-alignment). → G05, all fail; the LoRA result is meaningful (`artifacts/plans/archived/phases/05-06`, `outputs/group_a_failure/`).  ✓ verified (outputs/group_a_failure/{latent_cnn,latent_unet,frozen_feature_mlp})
