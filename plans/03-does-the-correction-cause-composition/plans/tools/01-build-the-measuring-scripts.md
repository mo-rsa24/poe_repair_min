# 🔧 Build and check the measuring scripts the rest of this scope uses

Build the thirteen scripts every later plan measures with, and run each one on a cached pair whose answer is already known.
...
|---|---|---|
| **1** | **this plan** | **✅** |
| 2 | ~~[fixing the size measure before any result](02-fix-the-size-measure-before-any-result.md)~~ | ✅ |
...
injects r_t at a chosen multiplier and inside a chosen step window **already
exists** and is already used by the residual_diagnostics experiments. What is
missing is a per-pair command line over it, real tests for the two conditions
that have to hold before any number from it is trusted, and the analysis
scripts. This plan adds those. It does not write a second sampler.
...
which adds r_t back scaled by λ. Specifically, already present:

| Needed for | Already there |
|---|---|
| the multiplier on the correction (plan 03) | `lambda_max`, plus `lambda_schedule` constant / linear_decay / early_only |
| the step window the correction applies in (plan 04) | `correction_window=(start, end)`, λ forced to 0 outside it (`_sampling.py:461`) |
...
| r_t is the [interaction term](../../../../context/world/interaction-term.md) | `extras["pmi_identity_residual_per_step"]`: per-step relative error of `Δ_t == w·(ε_J + ε_∅ − ε_A − ε_B)`, already computed |
...
Two facts decide how the must-pass checks have to be written:

- At `lam == 0.0` the loop takes the `eps_t = eps_poe` branch directly
  (`_sampling.py:549`) rather than computing `eps_poe + 0.0 * delta`. So λ=0 is
  exact by construction. The check still cannot be "bit-exact against
  `run_cfg_poe`", because that sampler batches three UNet branches where this
  one batches four, and the same UNet returns different numbers per batch shape
  (measured 1.95e-3 per step, compounding to 0.635 over 50 steps). Every one of
  these checks holds the batch shape fixed at four and varies only λ.
...
must-pass checks are written as tests that can go red. An assertion sitting in a
docstring does not count.

## Illustrations
The two command-line scripts are thin wrappers over the sampler that already
exists.
...
- Cache at `/datasets/mmolefe/poe_repair_min/outputs/training_cache/`.
  It holds **70 distinct pairs** across 76 pair directories (18 train + 58
  held-out).

  > Held-out means the pair was never trained on, so it is kept back to test
  > whether the fix reaches pairs the model has not seen.

  Six slugs are cached under BOTH splits
  ...
  One generated pair-and-seed run lives at
...
- Cache-only analyses and the must-pass checks run in-session on the current node.
  Anything invoking the UNet needs a GPU: biggpu, else bigbatch.
- Downstream checkpoint:
  `artifacts/scopes/does-the-fix-reach-unseen-pairs/pooled_lora/phase1_r8_100k/checkpoints/lora_step_100000.pt`.
...
- [x] `scripts/interaction_term_inject.py`: thin CLI over
      ...
      `--check-canary` (the λ=0 must-pass check). No new sampling logic.
- [x] `scripts/interaction_term_window.py`: same, exposing `--window
      start,end` and `--window off` (meaning `lambda_max=0`), plus
      `--check-identity`.
- [x] The must-pass checks, as tests in
      `tests/test_interaction_term_canaries.py`: 8 tests, all holding the UNet
      batch shape fixed. Each shown to fail against a deliberately broken
      sampler (two mutations, both reverted).
...
- [x] Run each measuring script on one cached pair and record the actual output
      in `report/instrument_smoke.md`.

## Success/Failure Outcomes
- **the cache scan**
  - Success: `70/70 ok`, all four eps keys at `[1,4,128,128]` fp16 everywhere.
  - Failure: name the file, quarantine it. Never silently skip, because a
    dropped pair-and-seed run biases every later average.
...
- **the λ=0 check**
  - Success: the sampler steps with eps_PoE itself, checked against its own
    ...
    ‖r_t‖. `run_cfg_poe` cannot be the reference here, because its batch shape
    differs, as above.
  - Failure: the injection path changes the sampler when it should do nothing,
    so every number measured at a higher multiplier afterwards is meaningless.
    Stop.
- **the window-off check**
  - Success: bit-exact against a full-strength run whose window sits past the
    last step. Same batch shape both sides, so only the window logic can differ.
  - Failure: same reasoning. Stop.
- **the checks can actually fail**
  - Success: breaking the code on purpose turns each test red.
  - Failure: a test that stays green against broken code proves nothing.
    Rewrite it.
- **analysis scripts**
  - Success: each runs on one pair and prints its headline number.
  - Failure: a script with no single-pair path cannot be checked cheaply. Add
    one.

## Recommended skill
▶ `/write-tests` ⚠️ for the two must-pass checks: the causal claim rests on them.
▶ `/demonstrate` ⚠️ once each script has run on one pair: show that pair's r_t
   beside its norm, so the quantity is visible before anything is built on it.
...
# the two must-pass checks, and proof they can fail
$PY -m pytest tests/test_interaction_term_canaries.py -v   # expect: all pass
cat report/instrument_smoke.md             # one recorded output per script
```
