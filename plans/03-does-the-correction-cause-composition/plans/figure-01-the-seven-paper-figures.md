...
This plan designs and builds the seven figures the main paper needs, each one built from real
scored runs and carrying its own control on the page.
...
| Step | Plan | Status |
|---|---|---|
| 12 | [baseline-01-the-size-matched-control-pool](../../04-does-the-fix-reach-unseen-pairs/plans/baseline-01-the-size-matched-control-pool.md) | ⚠️ |
...
> Low-rank means a few directions carry nearly all of the correction, so most of its 65536
> numbers are redundant and something small can reproduce it.
...
- [x] F2, more correction more composition: built as a 4x5 grid above four
      ...
      > AUC here is the area under the compose-rate-against-λ curve, on a
      > 0-to-1 scale. It is one number standing in for a whole curve, so rows
...
- [x] The two new control rows exist end to end: `--row wrong_seed` and
      ...
      `scripts/mechanism_study/run_dose_sweep_controls2.sh` (256 pair-and-seed
...
- [x] `scripts/snr_collapse.py` takes `--x-axis {log-snr,step}` and writes every
      curve, not just the median and the band. The step branch stacks the runs
...
- [x] F4 designed and built, as two figures rather than one.
      ...
      dog run in the window grid, 9 windows across and 4 seeds down, all 36 real
      ...
      layout said it should be. The run across window positions and the fork
      step disagree, and
...
- [x] D2 given its graded control, its hunt for a counter-example, and the
      reading it gets when nothing real is happening.
...
- [x] The counter-example hunt for D2, exhaustive and cache-only
      ...
      over steps 3 to 9, 0.01% after step 20, against 0.00% for random
      vectors. A
...
- [x] D3b, the controlled version of D2 and D3 together
      ...
      says which the correction is a function of. Runs with the same seed
...
- [x] Does the shared early component compose on its own? No, and the null is
      ...
      leave-one-out, 128 pair-and-seed runs) and scored 3% / 3% / 6% / 9% / 6%
      across λ = 0 to 1, AUC 0.059, against 0.387 for the pair's own real
...
- [ ] Two appendix figures that unfold F2's average over 32 runs, one task each.
...
- [ ] The overlap measurement, routed to `/frame-hypothesis` rather than to a
      figure of its own, since a half-right injection falsifies nothing and so
      cannot serve as an F2 control: compose rate against how much the correction's source pair
...
- [x] F5 designed and built: `python scripts/dial_vs_three_instruments.py` →
      ...
      from three measurements that already existed, each with the number it had
      to beat written down before its run. Where the picture sits (32 runs,
      0.031 to 0.992 along that run's own PoE-to-Mono axis, controls flat),
      what it is called (32 runs, two-animal readback 3.1% to 81.2%, crossing
      the blend rate at λ=0.75), and which way the correction pushes (38 runs,
      median +0.397
...
- [x] `/pair-figure` on F6's spectrum answered: one row is the correction at one
      denoising step of one run (one pair, one seed), flattened over the
      4x128x128 latent into 65536 numbers. Averaging a run over its timesteps
      is rejected. The adapter never produces such an average, since the LoRA is
      called once per step, and the steps of one run share no direction to
      average over, cosine +0.81 between steps 1 apart but +0.012 between steps
      20 to 49 apart, so the mean is set by whichever stretch carries the
      largest ‖r_t‖. The energy agrees: at 11 pairs and 8 seeds keeping every
      10th step, per step is 22.6% at k=8 against 2.1% for a same-shape
      Gaussian (10.7x), while averaging first gives 39.1% against 9.8% (4.0x),
      a higher number that claims less. Written into
...
- [ ] `/design-figure` F6, the correction is low-rank, so a small adapter can
      ...
      shaded underneath as the level to beat, and an inset showing how much of a
      held-out pair's correction the directions fitted on the training pairs
      explain.

      > Held-out means a pair the adapter never trained on. It is the only kind
      > that tests whether the fix reaches something new.
- [x] `/pair-figure` on F7's mechanism comparison answered: one point per pair,
      ...
      spread behind the diamond rather than averaged away. Rows inside a run
...
- [x] `/design-figure` F7 committed: three bands on one page sharing one pair
      ...
      What blocks this is one rescore rather than F8.
      `hypothesis-01-does-one-pooled-fix-transfer-at-all` already has a compose
      rate for these same eight pairs, but at checkpoint step 60000, while F7a's
      mechanism measurement used step 100000. Band two needs step 100000 scored
      for these eight pairs before it can sit beside F7a without comparing two
      different adapters. That plan's own task list already owes this ("score the
      checkpoints from step 70000 to 100000"), so it is a narrow scoring pass. It
      is neither a new run nor the fifteen-run leave-one-pair-out series (F8,
      `hypothesis-02-transfer-as-a-rate-over-fifteen-pairs`).
- [ ] `/evidence-ladder` build of the approved specs.
- [ ] The λ=0 numbers, owed to the results section and not to any figure: the
      compose rate over all 32 runs with nothing injected, and beside it the note
...
- [ ] Run the two illustration prompts (three-regime diagram, method schematic)
      ...
      `plans/03-does-the-correction-cause-composition/assets/`  [owner: human]
...
- [x] `language-score-as-the-correction-window-moves` given its register row, so it is no longer
      ...
      timing cliff is visible to a measuring tool that never counts animals),
...
the left, dog ear, muzzle and tongue on the right. Carrying one run through the
...
The second worked example is elephant × penguin. It appears only where a figure has to answer
