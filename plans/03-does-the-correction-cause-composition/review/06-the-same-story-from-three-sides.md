...
[../plans/hypothesis-05-the-same-story-from-three-sides.md](../plans/hypothesis/06-the-same-story-from-three-sides.md).
One of its answers fills **F5** in the figure register; the rest support the central claim rather
than carrying a figure of their own.

The two image-side checks agree with the run across correction strengths and share none of its
scoring machinery.
...
| File | What it holds |
|---|---|
| [design](../plans/hypothesis/06-the-same-story-from-three-sides.md) | the five checks, their thresholds, and why each shares no machinery with the run across strengths |
...
**There is no single threshold here, and inventing one would misrepresent the plan.** Each of the
five checks carries its own threshold, written before the run and stated inside its own question
below. None of them may overturn the central result from the run across strengths on its own,
since a disagreement is a diagnosis rather than a verdict. What the set buys is that the two
image-side checks agree with that run while sharing none of its scoring machinery.
...
the threshold rather than inside it).
...
- [x] ❌ Does the additivity gap predict how big a correction each pair needs?
      **No.** Over 75 pairs, the rank correlation against the preregistered correction size
      ...
      sequence, and +0.32 for the concatenation. The threshold was |ρ| ≥ 0.30 with p ≤ 0.05,
      written before the run; exactly one of the four views squeaks past it and the CLIP-L view sees
...
- [x] ❌ Do all pairs leave the same thing behind when the two separate prompts are subtracted?
      **No, not beyond what the prompt template already explains.** Against an isotropic random
      baseline the answer looks emphatic. The top direction holds 18-20% of the residual energy
      against a 1.5-2.0% baseline, a 10-12× margin. That baseline is the wrong comparison. Every
      prompt here has the same shape ("a X and a Y" against "a X" and "a Y"), so a shared
      direction can be the "and", the extra length, or the anisotropy every CLIP text embedding
      carries. Rebuilding the residual with the second solo prompt taken from a **different pair**
      keeps all of that and destroys only the binding. That control reaches 12.8-15.2%, so the
      real pairs beat it by only 1.19-1.57× against a 2.0× threshold. The CLIP-L sequence
      embeddings are nearly parallel across all prompts to begin with (mean pairwise cosine
      +0.999), which is where most of the apparent sharing comes from.
      No language-space twin of the low-rank ‖r_t‖ claim survives.

      > Low-rank here means that a few directions carry most of the energy in a stack of
      > vectors, which is what makes a small adapter able to learn them.
- [x] ✅ Is a blended animal wrong content rather than a poor-quality picture?
      **Yes, and this removes the objection.** Over 749 paired runs (same seed, same prompt,
      same sampler, the [interaction term](../../../context/world/interaction-term.md) the only
      difference) the content changes hard and the quality does not.
      [Compose rate](../../../context/world/compose-rate.md) 13.9% → 48.9%, 323 runs gained
      against 61 lost, McNemar p = 3.5e-44. The four content-blind quality proxies all sit inside
      the ±0.20 sd threshold: Crete blur −0.13, noise −0.05, contrast +0.04, colourfulness +0.20.
      Two cautions. Colourfulness lands on the threshold rather than comfortably inside it. And two
...
- [x] ✅ Does the caption readback cross over from a blend description to a two-animal
      ...
      what the cached true correction gains.
...
- [x] ✅ **Was the comparison fair?** The quality check is the strongest case: 749 paired runs at
      the same seed, same prompt, same sampler, with the interaction term the only difference. The
      binding-direction check went further and replaced its own baseline, because the isotropic
...
|---|---|---|
| why text anchoring separates blends from compositions where image-to-image distance does not | a direct comparison of the two reads on the same runs | nothing is blocked, but both results are currently used without knowing why one works |
| whether the mis-aimed correction's 44% is a real margin or noise | more runs on that control | F5's caption, which currently leans on the weakest number in the plan |
