...
[../plans/hypothesis-03-when-in-the-run-it-matters.md](../plans/hypothesis/05-when-in-the-run-it-matters.md),
and its answers fill **F4** in the figure register, the figure showing when in the denoising run
...
| File | What it holds |
|---|---|
| [design](../plans/hypothesis/05-when-in-the-run-it-matters.md) | the sliding window, the grid, the threshold the result has to beat |
| **this file** | **the verdict: timing decides, and it decides at the very start** |
| [the independent estimate](04-what-the-cached-runs-already-show.md) | the fork step, step 16, which disagrees with this and is not a contradiction |
...
**Tests the claim.** A result that misses the threshold below closes the plan and opens one
follow-on.
...
|---|---|---|---|---|---|
| Leak check, `--window off --check-identity`, a_cat×a_dog seed 9, 50 steps | Checks the runner | 2026-08-10, in-session mscluster GPU 1 | 1 run | stdout only | done, passed |
...
- [x] ✅ Does the [compose rate](../../../context/world/compose-rate.md) peak at some window
      position rather than staying flat?
      Yes, and more sharply than the threshold expected. The curve does not peak in the middle of
      the run. It falls off a cliff right at the start. The earliest window, steps 0 to 10,
      composes 0.656 of its 32 runs. One notch later, steps 5 to 15, it is 0.250. By steps 20 to
      30 it is 0.000 and it stays there. Timing matters, and it matters at the very beginning.
      ...
      needs a narrower window near the start rather than more window positions further along.
...
- [x] ✅ Does the peak sit where the correction is largest?
      ...
      This is the answer the threshold hoped for, arrived at by a different route. The threshold
      assumed size would be nearly flat and so could not pick out a moment. Size is not flat. It
      rises through the run, and it still does not pick out the moment, because it rises where the
      compose rate falls. Either way size is ruled out as the explanation, and the stronger
      sentence is now available. What decides the outcome is when the correction lands, and its
      size where it lands does not.
...
- [x] ✅ Does the peak land near step 16, the fork step measured from cached data?
      ...
      doing its work. Moving the window across the run asks something else: where the correction
      has to be applied for the outcome to change. A cause acting early and a difference becoming
      visible later is the ordinary relationship between the two, so this does not call the run
      that measured step 16 into question.
...
- [x] ✅ Does the whole nine-window curve keep its shape once every window delivers the same
      total? Yes. Thirty-six runs, four per window (`--mode matched`,
      ...
      constant moves two points by one run each and does not touch the shape.
...
|---|---|---|
| whether the true best window sits earlier than steps 0 to 10 | a narrower window near the start rather than more window positions further along. A ten-step window cannot begin before step 0 | nothing is blocked; F4's caption states the limit instead |
...
| why the detector disagrees with the eye on cat × dog | comparing `dose_matched/swap_scores.json` against the images run by run | nothing yet, but every claim on this pair currently rests on an eye read rather than a count |
