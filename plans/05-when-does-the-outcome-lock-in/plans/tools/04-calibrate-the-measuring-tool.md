# 🔬 Calibrate the measuring tool

**What this plan asks:** does the endpoint predictor agree with the teacher well enough, and
equally well across the three trajectory families, to be trusted on the whole grid?

Step 47 in the root running order; waits on step 46 (the one-state check must pass); next is
[the grid and the figures](../figures/05-the-grid-and-the-figures.md).
...
— custom; no skill fits (an agreement pass with thresholds already designed).
...
|------|------|-------------|
| 46 (previous) | [wire the endpoint predictor](03-wire-the-endpoint-predictor.md) | the adapter and its one-state check |
| **47 (current)** | **Calibrate the measuring tool** | **buys or refuses trust: 240 states, predictor against teacher, two thresholds in code, per family** |
| 48 (next) | [the grid and the figures](../figures/05-the-grid-and-the-figures.md) | the two prompt passes and the paper figures |
...
seeds, 5 spread steps), compare the endpoint predictor's settled frame with the teacher's
finish-the-run ending, scoring agreement as scorer-verdict match plus DINOv2 distance, at
guidance 7.5.

> The **endpoint predictor** is LCM-SDXL behind plan 03's adapter, jumping a state straight to
> its ending. The **teacher** is SDXL finishing the run properly from the same state, which is
> the ground truth.

**The hypothesis.** The distilled student tracks its teacher well enough to measure the
grid with, and comparably across families. If true, plan 05 runs on the predictor at about one
...
comparability is a threshold, never an assumption.

**Context details.** Teacher endings by plan 01's script; predictor by plan 03's module; scorer
is the validated instance-count scorer. Distilled students are documented to memorize, which
this pass would catch.

**This plan's job.** Turn "good enough to measure with" from a disclaimer into a recorded
verdict: adopt, shrink, or fall back.

**Associated materials.** Verdict: [the review file](../../review/04-calibrate-the-measuring-tool.md).
...
**Prerequisites.** Plan 03's one-state check passed; the cache present; the scorer importable.
...
against two thresholds that sit in source before any number exists.**

**Why this matters right now:** every plan-05 figure inherits this verdict; without it the
predictor's frames carry false authority.
...
**Key insights:**
1. Two thresholds, not one: an absolute minimum (`AGREEMENT_FLOOR`), and a comparability
   requirement (`FAMILY_GAP_MAX`), because a per-family bias is the confound that corrupts
   cross-family comparison even when average agreement looks fine.
...
```
 240 states: 3 families x 8 pairs x 2 seeds x 5 steps
      |                          |
 endpoint predictor          teacher finish (~100 calls)
 (1-4 calls)                     |
      +---- agreement per state -+
              |
   per family: scorer-match rate, DINOv2 distance
              |
   threshold 1: minimum per family
   threshold 2: max gap between families
              |
...
1. **The calibration script** `scripts/commitment/calibrate_oracle.py`: builds the subsample
   ...
   rates, the two thresholds, the verdict). Thresholds `AGREEMENT_FLOOR` and `FAMILY_GAP_MAX`
   in source.
...
2. `verdict.json` records adopt, shrink, or fall back, and the review file's question is answered.
...
[what plan 03 had to pass](03-wire-the-endpoint-predictor.md#what-has-to-pass-before-this-runs).
...
3.1 **Open W&B** at wandb.ai, project `prime_lab/poe-repair-animals-compose`, find the
   ...
   - ✅ If both thresholds pass, record "adopt" in the review file and mark it passed.
   - ❌ If the minimum fails for a family, record which, and whether "shrink" (drop that family
     or its failing steps) preserves the figures plan 05 owes; record the choice.
   - ❌ If the family gap exceeds its threshold, record "fall back": plan 05 runs finish-the-run.
...
▶ **Next: what has to pass before this runs**, then [plan 05](../figures/05-the-grid-and-the-figures.md).

## What has to pass before this runs

Navigation: ⬅️ [Previous](#instructions) | 📋 [TOC](#table-of-contents) | [Next](#figure-catalog) ➡️

> **This is where the scope decides what it trusts: plan 05's choice of measuring tool is made
> here, and figures built on an uncalibrated predictor would carry false authority into the
> paper.**
...
- **Fail criteria (not a stop; a fork):**
  - A failed minimum or gap sends plan 05 down its named fallback (shrink, or finish-the-run
    everywhere); the verdict is recorded either way and the scope continues.

**When you get results, answer** [the review file](../../review/04-calibrate-the-measuring-tool.md).
...
|------|------|-------------|---------------|---------|
| The calibration read | subject | [diagram-prompts.md](../../diagram-prompts.md#prompt-4-subject-the-calibration-gauge) | student against teacher, two thresholds | `../diagrams/when-does-the-outcome-lock-in-04-the-calibration-gauge.png` |
...
|------|------|-------------|--------------|--------|---------|
| Per-family agreement chart | — | agreement rate per family with both thresholds drawn | `calibrate_oracle.py` | ⏳ | filed to `artifacts/results/when-does-the-outcome-lock-in/oracle-agreement-per-family__240-state-calibration.png` |

#### Organization workflow

1. Run; 2. Read in W&B; 3. File the per-family agreement chart with its card; 4. Link here.
...
```python
AGREEMENT_FLOOR = ...   # set before any run; the absolute per-family minimum
...
**File:** `poe_repair/lcm_oracle.py` (from [plan 03](03-wire-the-endpoint-predictor.md))

## Next step

Navigation: ⬅️ [Previous](#code-references) | 📋 [TOC](#table-of-contents)

[The grid and the figures](../figures/05-the-grid-and-the-figures.md), on whichever measuring tool this
