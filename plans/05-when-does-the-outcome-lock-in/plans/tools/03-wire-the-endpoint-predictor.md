# 🔬 Wire the endpoint predictor

**What this plan asks:** can a cached state be handed to LCM-SDXL and come back as the image
that state would have finished as, with the wiring proven right on one state?

Step 46 in the root running order; waits on step 44 (the premise must stand); next is
[calibration](04-calibrate-the-measuring-tool.md).

## Recommended prompt (after this plan completes)

```
/sync-plan-tree @plans/05-when-does-the-outcome-lock-in/plans/03-wire-the-oracle.md — <one line on what actually happened>
```
...
|------|------|-------------|
| 45 (previous) | [the free test](../tests/02-the-free-test.md) | the first of the two tests, from the cache alone |
| **46 (current)** | **Wire the endpoint predictor** | **LCM-SDXL downloaded, adapted to cached states, and checked once against one teacher ending** |
| 47 (next) | [calibrate the measuring tool](04-calibrate-the-measuring-tool.md) | the 240-state agreement pass |
...
> The **endpoint predictor** is that adapter plus LCM-SDXL behind it. It jumps a state straight
> to its ending instead of stepping the rest of the way. The **teacher** is SDXL itself finishing
> the run properly from the same state, which is the ground truth the predictor is judged against.
...
**This plan's job.** Make the tool exist and pass one short check on a single state, so
calibration has something to judge.

**Associated materials.** Verdict: [the review file](../../review/03-wire-the-endpoint-predictor.md). The
...
**Expected runtime.** Download a few GB once; the one-state check itself is under a minute of GPU.

**Prerequisites.** Plan 01 passed its check; a GPU node; disk room on `/datasets` for the
...
**One adapter module and one passed check: a cached state in, a settled frame out, agreeing
...
timestep mapping exact), one check against ground truth.

**Key insights:**
1. The teacher ending for that check comes from plan 01's finish-the-run script, so the ground
   truth costs nothing new.
2. The prompt is an argument, never hardcoded: both prompt passes (joint prompt, expert pair)
   call this same module in plan 05.
...
3. **The one-state check** `scripts/commitment/lcm_smoke.py`: one pair-and-seed run's step-25
   state through the predictor (joint prompt) beside the teacher's finish from plan 01's script;
   scorer verdicts and DINOv2 distance printed; thresholds `SMOKE_SCORER_MATCH = True` and
   `SMOKE_DINO_DIST_MAX` in source.
...
pre-set thresholds; this plan builds the thing those thresholds judge.

**Goals:**
1. The adapter exists with its three asserts, and the one-state check passes its threshold.
...
[what plan 01 had to pass](../reading/01-basins-by-hand.md#what-has-to-pass-before-this-runs), so the premise stands.
...
▶ **Next: [task 2.1](#2--run-the-one-state-check)**.

### 2. 🚀 Run the one-state check

◀ **Needs: [tasks 1.1 to 1.2](#1--download-and-adapt)**.

- [ ] **2.1** Run the check on a single state.
  ...
  - **Done when:** it prints its scorer verdicts and DINOv2 distance, the side-by-side
    PNG exists, and the pass or fail against the threshold is recorded in the review file.
...
- [ ] **Capture the failures this plan hit.**
  - Paste: `/ingest-error-pattern --from-run-log @plans/05-when-does-the-outcome-lock-in/plans/03-wire-the-oracle.md`
...
◀ **Needs: [task 2.1](#2--run-the-one-state-check)** done, so the side-by-side exists.

3.1 **Open the side-by-side PNG** under
   `/datasets/mmolefe/poe_repair_min/outputs/commitment/lcm_smoke/`.
   - Expected result: the predictor's frame and the teacher's ending, same subject, comparable
...
▶ **Next: what has to pass before this runs**, then [plan 04](04-calibrate-the-measuring-tool.md).

## What has to pass before this runs

Navigation: ⬅️ [Previous](#instructions) | 📋 [TOC](#table-of-contents) | [Next](#figure-catalog) ➡️

> **A failure caught here is cheap; the same failure discovered inside plan 05's grid is not.**

- **Pass criteria:**
  - All three asserts hold on a real call.
  - The scorer verdicts match and the DINOv2 distance is under its threshold, and the eyeball
    read concurs.
- **Fail criteria (STOP):**
  - Any assert fires, or the two frames disagree. Fix the adapter or record the checkpoint as
    unusable; calibration does not run on a failed check.

**When you get results, answer** [the review file](../../review/03-wire-the-endpoint-predictor.md).
...
|------|------|-------------|---------------|---------|
| The endpoint predictor | subject | [diagram-prompts.md](../../diagram-prompts.md#prompt-2-subject-the-endpoint-spyglass) | the predictor reading a state ahead | `../diagrams/when-does-the-outcome-lock-in-02-the-endpoint-spyglass.png` |
...
|------|------|-------------|--------------|--------|---------|
| The one-state side-by-side | — | predictor frame beside teacher ending, one state | `lcm_smoke.py` | ⏳ | filed to `artifacts/results/when-does-the-outcome-lock-in/oracle-vs-teacher-ending__one-state-smoke.png` |
...
|------|---------|--------------|---------|
| Check the plan | `/verify-plan @plans/05-when-does-the-outcome-lock-in/plans/03-wire-the-oracle.md` | **task 0.1** | conformance reported |
| Capture patterns | `/ingest-error-pattern --from-run-log @plans/05-when-does-the-outcome-lock-in/plans/03-wire-the-oracle.md` | **the close out** | errors catalogued |
| Bring the tree current | `/sync-plan-tree @plans/05-when-does-the-outcome-lock-in/plans/03-wire-the-oracle.md` | **the close out** | statuses match reality |
...
[Calibrate the measuring tool](04-calibrate-the-measuring-tool.md): 240 states, two thresholds, the
verdict that decides whether plan 05 runs on the predictor or on the teacher.
