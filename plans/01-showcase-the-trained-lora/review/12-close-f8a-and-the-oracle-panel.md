# 📊 Review: did anything move after 60k, and is the ceiling crisp?
...
- **The tail**: checkpoints 70k to 100k, sampled every 10k, whose renders exist and were never
  scored.
- **The ceiling panel**: LoRA-corrected beside true-r_t-injected renders on the same cells;
  qualitative because no crispness instrument exists.

## Runs

Navigation: ⬅️ [Words](#words-this-file-uses) | 📋 [TOC](#table-of-contents) | [Next](#the-pre-registered-bar) ➡️

| Date | What ran | Cells | Wall time | Outcome |
|---|---|---|---|---|
| | | | | |
...
- [ ] ⚠️ **Does out_out compose rate at 70k-100k stay inside the band the 10k-60k points draw?**
  Bar, fixed before scoring: inside means saturation confirmed and plan 08's framing stands; a
  late rise beyond the existing points' spread reopens train-longer and is flagged to plan 08
  before it launches.
...
- [ ] ⚠️ Is the oracle visibly crisper than the LoRA on the F9 cells, or equally soft? (The
  eye's answer, cell by cell; this is experiment B's interpretation key.)
...
- [ ] ⚠️ **Was the instrument sound?** Same validated scorer as the earlier table.
