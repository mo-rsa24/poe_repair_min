...
```
/sync-plan-tree plans/01-showcase-the-trained-lora/plans/figures/04-the-transfer-matrix-figure.md — <one line on what actually happened>
```
...
📋 [TOC](#table-of-contents) | [Next](#quick-context-where-you-are) ➡️
...
```
groups (pooled checkpoints)      disjoint evaluation pairs
        │                                 │
        └──────────────┬──────────────────┘
                        ▼
              1.1 census the squares
              (every group × every candidate pair)
                        │
                        ▼
              1.2 audit disjointness, token by token
              ┌─────────┴─────────┐
              ▼                   ▼
         disjoint square     shared-token square
         → scored               → excluded, marked `within`
              │
              ▼
              1.3 fill: run the uncached disjoint squares
              (seeds × 50 DDIM steps, window 0–10, λ=1)
              │
              ▼
              1.4 score + render
              rows: training group
              columns: disjoint evaluation pair
              each cell: compose rate, plain-PoE baseline row beneath
              │
              ▼
              the transfer matrix (step-34_transfer-matrix.png)
```

This is the process view of the same piece the illustrated map calls "the transfer matrix"
(subject lane, [diagram-prompts.md, prompt 4](../../diagram-prompts.md)): a grid of outcome
tiles, dashed until scored, with a scorer lens over one tile and no diagonal drawn as a claim.
...
| Figure | Lane | Description | Generated by | Status | Axes |
|---|---|---|---|---|---|
| step-34_transfer-matrix.png | subject | compose rate per (train group, disjoint pair) | task 1.4 | ⏳ | rows: training group; columns: evaluation pair; each square: compose rate 0 to 1, plain-PoE baseline row beneath |
