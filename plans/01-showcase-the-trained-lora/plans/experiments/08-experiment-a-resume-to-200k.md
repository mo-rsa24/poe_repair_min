...
**This plan asks one question: does doubling the training steps from 100k to 200k change anything the tracking set can see?**

**Step 38 in the root running order. Waits on: step 36 (the frozen tracking set; step 31's verdict arrives through it). Next: [09-experiment-b-rank-16-32](09-experiment-b-rank-16-32.md).**
...
**The hypothesis:** *training length is not the knob: doubling the steps moves neither the held-out [compose rate](../../../../context/world/compose-rate.md) outside its seed-noise band nor the frozen tracking set's crispness reads.*
...
**Prerequisites:** plan 06's frozen tracking set (hash in config); plan 01's verdict on where the training curve stops rising, recorded.
...
- biggpu allows one Slurm job per user; this run takes that one job ([overview](../../../../environment/overview.md)).
...
**The length axis settled by a run whose null threshold was fixed before launch.** A null here is a finding: it closes "just train longer" with a number instead of an opinion.
...
**The solution.** One resume, one axis, the frozen tracking set watching.
...
2. The comparison table written; the review file's threshold question answered.
...
- [ ] **0.1 Run the following prompt: `/verify-plan plans/01-showcase-the-trained-lora/plans/experiments/08-experiment-a-resume-to-200k.md`**

▶ **Next: [task 1.1](#1--launch-and-harvest)**.

### 1. 🧪 Launch and harvest

◀ **Needs: [06 instruction 2.3](../tools/06-extend-the-tracking-set.md#2--prove-the-first-short-run-in-wb)** (frozen tracking set proven) and **[01 instruction 2.4](../reading/01-read-where-the-curves-flatten.md#2--read-the-curves-in-wb)** (the ceiling-or-waypoint verdict recorded).
...
2.2 **Capture the tracking panel** at completion into the place `runbook/looking-at-what-a-run-produced/reading-a-training-run.md` keeps for screenshots (wandb or Playwright MCP), captioned.
...
> The null threshold was fixed before launch. Whatever lands, the review records it against that threshold; the threshold does not move to meet the result.

**Pass criteria:**
- Run reached 200k; the comparison table complete; the review file's threshold question answered.

**Fail criteria:**
- The run died before 200k (harvest, ingest, relaunch decision in the review), or the tracking manifest hash changed mid-run (the tracking set unfroze; the comparison is void).
...
|--------|------|---------------|---------|
| (none owed; A's panels live in W&B and feed plan 12's reading of what the cached correction can reach at best) | — | — | — |
...
2. Execute the plan (task 1.3 writes `experiment_a_verdict_inputs.json`).
3. Organize outputs: file the JSON beside the review file's Runs table entry.
4. Link the finished JSON above once it lands.
...
[09-experiment-b-rank-16-32](09-experiment-b-rank-16-32.md): the capacity axis, over the SSH-plus-nohup path while A holds the one Slurm job.
