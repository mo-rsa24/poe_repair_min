...
- **Review questions:** [the review file](../../review/01-read-where-the-curves-flatten.md)
...
- [ ] **0.1 Run the following prompt: `/verify-plan plans/01-showcase-the-trained-lora/plans/reading/01-read-where-the-curves-flatten.md`**
...
2.3 **Screenshot the panel** into the place kept for it in [runbook/looking-at-what-a-run-produced/reading-a-training-run.md](../../../../runbook/looking-at-what-a-run-produced/reading-a-training-run.md).

2.4 **Write the verdict** into the [review file](../../review/01-read-where-the-curves-flatten.md): ceiling or waypoint, the slope, the span, the run id.
...
**When you have the verdict, answer the open questions in the [review file](../../review/01-read-where-the-curves-flatten.md).**
...
|--------|------|-------------|----------------|---------|
| reading where the curve flattens | subject | [diagram-prompts.md#prompt-1-subject-reading-where-the-curve-flattens-planned](../../diagram-prompts.md#prompt-1-subject-reading-where-the-curve-flattens-planned) | two rising-then-flat curves (the fraction-of-distance metric and the loss) with the ceiling/waypoint exit split | `diagrams/reading-where-the-curve-flattens.png` |
...
1. Render the pending subject-lane piece via `/render-diagrams` once the ceiling-or-waypoint verdict is recorded, so the exit split it draws matches the real outcome.
2. Run the plan's Tasks and Instructions.
3. Confirm the sidecar json lands at the path task 1.1 names.
4. Link the rendered diagram and the sidecar here once both exist.
...
```
Execute plans/01-showcase-the-trained-lora/plans/reading/01-read-where-the-curves-flatten.md: run task 0.1's verify-plan, then the Tasks lane; stop at the handoff to the manual W&B read.
```
