...
[the plan that builds the clean pair pool](../plans/tools/01-the-clean-pair-pool.md),
the check the whole transfer claim starts behind. Every later run in this scope draws its pairs
...
| File | What it holds |
|---|---|
| [design](../plans/tools/01-the-clean-pair-pool.md) | how the pool is built, and the checks it has to survive |
| **this file** | **the verdict: the pool is clean, with one control pair in dispute** |
| [what it unblocks](03-does-one-pooled-fix-transfer-at-all.md) | every run in this scope draws its pairs from here |
...
**Not a run: a measuring tool.** Judged by whether its checks could have failed, not by what they
...
- [x] ✅ Does every training pair actually fail by default?
      ...
      > Held-out means a pair the adapter never trains on, so a result on it says whether the fix
...
[hypothesis-02 of the other claim](../../03-does-the-correction-cause-composition/review/03-more-correction-more-composition.md).
...
|---|---|---|
| whether elephant×penguin composes by default | re-scoring that pair under both runs' conditions and finding what differs between them | the do-no-harm claim, the limitations sentence in `writing-06-mechanism-and-limitations`, and reading the fifteen runs as do-no-harm-clean |
