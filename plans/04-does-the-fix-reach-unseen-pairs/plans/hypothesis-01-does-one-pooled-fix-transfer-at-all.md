...
Train a single adapter on all the training pairs and see whether it still helps on pairs it
never saw, before paying for fifteen separate training runs.
...
Design only. Verdicts live in [the review file for this plan](../review/hypothesis-01-does-one-pooled-fix-transfer-at-all.md).
...
Training fifteen LoRAs, each leaving one pair out (plan 03), is expensive. One pooled
LoRA trained on all training pairs and evaluated on a few unseen blend pairs answers the
scope's core question far more cheaply: does the fix transfer at all? A positive or
ambiguous Phase-1 result justifies Phase-2, the full leave-one-pair-out set. A flat null
...
reference), and the compose-by-default control. Read
[compose-rate](../../../context/world/compose-rate.md) for in-distribution (`in_in`)
against held-out (`out_out`). This is a single-LoRA precursor to plan 03's
leave-one-pair-out set, and does not replace it.

## Purpose
Serves Objective 2 (Transfer A) as a cheap first cut. Its result decides whether plan 03's
15-run leave-one-pair-out set is worth running.

## Goal
One pooled LoRA trained, held-out compose-rate read on the best checkpoint, and a
go/no-go call on Phase-2, the leave-one-pair-out set in plan 03.
...
- **No better than plain PoE**: a held-out pair whose compose rate matches what plain PoE
  already gets, so the adapter bought nothing there. Two very different things cause it,
  and telling them apart is why the two extra measures exist: either the fix never arrived
  (it was not delivered), or it arrived pointing the wrong way (it did not transfer).
...
- [ ] Re-read the pairs that came out no better than plain PoE on the two direction
      measures, once `instrument-02-three-live-curves-while-training`'s first short run is
      green, so each such pair can be told apart by which of the two causes above it is.
- [ ] Write the go/no-go note for the fifteen training runs into the review file, with the
...
3. Re-read the pairs that came out no better than plain PoE on the direction measures, once
   plan 02's first short run is green.
4. Write the go/no-go note for the 15 training runs into the review file.
...
MUST PASS before this counts as done: the pooled run wrote a compose_rate.json with both
`in_in` and `out_out` filled in on the final checkpoint, and a one-line Phase-2 go/no-go is
recorded. A script asserts both keys are present and non-empty.
STOP: if the held-out `out_out` compose-rate is no better than the plain-PoE fail-rate AND
the direction measure comes out wrong once wired, mark Phase-1 a no-transfer null and do NOT
launch the 15 leave-one-pair-out runs, because the cheap pass has already answered it.

## Recommended skill
▶ `/run-experiment` ✅: drives the final-checkpoint scoring pass. alt: `/analyze-run` to
   walk the pooled run's checkpoints for the converged held-out read.
