...
This file designs the one measurement that splits the correction into the part a better sampler
could remove and the part only the model can account for.
...
targeting the product of two marginals. That is a legitimate test and an off-distribution
...
whose displacement sits below the minimum written in source means the measuring tool failed, and
nothing about the answer can be read off it.

**The trend is the result, and only once the curve has flattened.**
...
| the chain diverged and the latents are garbage | latent norm bound in the step-size search, and the composing-pair condition (task 3, task 4) |
| the joint branch degrades off-distribution, so any rise comes from the test rather than from the pair | the composing-pair condition, where the corrector's target is close to the joint and the curve should stay low at all `k` |
| the corrector never moved | the displacement column, with a minimum written in source |
| the chain has not equilibrated at the largest `k` | the `k=100` against `k=200` comparison, with an instability bound in source |

**Falsify condition, three-way, at the thresholds written before the run.** They live in source as
constants in `scripts/corrector_residual_curve.py`, following the `MIN_MEDIAN_RATIO` pattern, so
they cannot be moved after the answer is visible.

- **Support, the split is real.** Over the last five denoising steps, the ratio at the `k` where
  the curve has flattened has fallen by at least `MIN_DROP_FOR_SPLIT = 0.20` of its `k=0` value. At least
...
- **Null, it is all the model.** At every step the change between `k=0` and the flattened `k` is
...
- **Inconclusive.** The ratio still moves by more than `MAX_K_INSTABILITY = 0.05` between `k=100`
  ...
  Return to task 3 and widen the search. Never loosen a threshold.
...
|---|---|
| mostly error B | the framing strengthens. The correction is a model-level object, the adapter is the right tool for it, and the sampler comparison becomes a two-sentence related-work note |
...
not commute. So at high noise the smallest the size can get is the non-commutation gap plus the
diffused model gap, and no amount of `k` separates them. The early part of the curve gives a size
only, and cannot say which of the two errors that size came from.
...
Three things, in two halves, and the second half only starts if the first half's measurement comes
back a certain way.
...
the condition between them is written into the tasks rather than left to judgement.
...
corrector can be on inside a range and off elsewhere, matching the interface used when the `r_t`
window was slid across the run.
...
The arithmetic, so a first short run can check it rather than a guess standing in for it. Guided
...
still bounded away from zero at the last denoising step is already a lower limit on error B,
without a single new sample.
...
- [ ] **1.2** State three things in the review file or the read is not worth having. First, the
      ...
      about that path only. Third, whether both predictions were evaluated at the same latent at each
...
- [ ] **1.3** Write the lower limit into the review file as a number with its unit and its
      meaning, and say what it does to the claim. A large limit means error B is already big and
      the `k` grid is a sizing exercise. A limit near zero, honestly measured, is the first
      evidence for error A and raises the stakes on everything below.
...
Run group: **hypothesis**, the measuring-tool half.
...
- [ ] **2.2** The leak check, before any measurement. `k=0` must reproduce plain
      ...
      batch shape.
- [ ] **2.3** A second leak check the window grid did not need: with a corrector window placed
...
Run group: **hypothesis**, the measuring-tool half. This gets a task of its own because at the
wrong step size the chain either never moves or diverges, and both of those imitate a scientific
answer.

◀ **Needs: task 2.2**, so a moving chain can be distinguished from a broken composer.

- [ ] **3.1** Run the multiplier `c` at all five of `{0.01, 0.035, 0.1, 0.3, 1.0}` at `k=20` on
      `a_cat__x__a_dog` seed 9, where `δ_t = c·β_t`. The vendored sampler's 0.035 at `k=20` sits at
      the centre of the range and is tested rather than assumed.
...
▶ **Next: task 4.1**, the measurement everything else hangs on.

### 4. 📊 What is left once the chain settles

Run group: **hypothesis**. This is the plan's load-bearing measurement and its thresholds are
already written in [The claim](#the-claim). No images, no detector, no scorer.

◀ **Needs: task 3.3**, the fixed step size.

- [ ] **4.1** Write `scripts/corrector_residual_curve.py` with the five thresholds as module-level
...
- [ ] **4.2** A first short run in-session: one pair, one seed, `k ∈ {0, 1}` only, and check the measured
...
- [ ] **5.1** Recreate the sliding-window grid with the corrector at the `k` on the flat part of
      ...
      positions as columns, green border on the pictures the detector scored as composed, plus a tenth column with the
...
**The condition on this task is deliberately soft.** A flat curve in task 4 does not strictly imply no
...
that it was run even though task 4's curve came back flat.
...
### 8. 📖 Feynman-Kac correctors, only if the corrector moved something

Run group: **idea**. Blocked on a full read because no usable implementation was found.

◀ **Needs: tasks 4.5 and 7.4**. Run this only if the corrector moved something. If task 4.5
...
- [ ] **9.2** Confirm the picked `c` sits in the middle of the usable range rather than at its
      ...
      to widen the range rather than accept the edge.
...
### 10. 📊 Judge the curve by eye against the printed verdict
...
- [ ] **10.1** Open
      ...
      not equilibrated whatever the threshold printed, and the honest answer is inconclusive.
...
- [ ] **10.3** Look at the composing pair's panel. If its curve behaves like the failing pair's,
      the test is measuring itself and the whole grid is void. Record that judgement even when it
...
Between task 4 and task 5, nothing proceeds until the three-way threshold has fired and the branch
is written down.
...
|---|---|---|
| `mcmc/how-much-of-the-correction-a-corrector-removes.png` | y is `‖r_t^(k)‖/‖eps_PoE‖`, x is denoising step 0 to 49, one curve per `k ∈ {0,1,5,20,100,200}`, one panel per pair, second row carrying the two norms separately. From `corrector/residual_curves.json` | **yes**, if the check at task 4 passes. It is the dynamics of the correction under a corrector, which is the whole question |
| `mcmc/samples-as-a-ten-step-corrector-window-slides.png` | rows are seeds 9 to 12, columns are the nine window positions plus a corrector-on-all-50 column, each entry is the final picture, green border where the detector scored composed | **yes**, if it differs from the injected-`r_t` version. Identical behaviour is one sentence of prose beside the existing figure |
...
| `how-much-is-added/across-composition-rules/rules-as-the-dose-rises.png` | rows are the four rules, columns are `λ ∈ {0,0.25,0.5,0.75,1}` at seed 9, `λ=1` entries labelled not-an-identity where the rule cannot reach it | **yes**, if the check at task 4 passes. The `λ=1` column separates rules that combine scores from rules that do something else |
| the step-size search | the five `c` values against displacement and latent norm | no. It belongs in the review file, since it measures the measuring tool rather than the phenomenon |
...
that is a deliberate part of the design. The residual norm stands in for the distributional gap
without being that gap.
...
score model with no energy head. That is why the chain is judged to have settled by the curve
flattening in `k` plus a displacement column rather than by an acceptance rate, which would have
been the cheaper diagnostic.
...
**More than one seed on the curve.** Task 4 is a diagnostic rather than a rate, and seeds cost the
...
**The corrector as a paper baseline.** If task 4 returns a null, the corrector is not a method
this paper competes with and no compose-rate condition is owed. Tasks 6 and 7 still run, because
...
| [scripts/interaction_term_dose_matched.py](../../../scripts/interaction_term_dose_matched.py) | the `--mode matched` setting task 7.4 reuses so rows deliver comparable totals |
...
error B has a lower limit before a single corrector step runs.
...
| the ratio rises monotonically with `k` | step size too large and the chain is diverging, or the joint branch degrading off-distribution | the latent norm against 1.5× the uncorrected latent, then the composing-pair panel | the first is a smaller `c`; the second voids the test and needs a different diagnostic |
| the grid finished but `residual_curves.json` has fewer than 600 rows | a level was skipped, or a `k` value crashed and the loop swallowed it | row count per `(pair, k)` | rerun the missing runs; the script is resumable by design |
