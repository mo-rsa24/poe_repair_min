...
This scope asks how much of the correction `r_t` is the sampler's fault, which a Markov chain can
...
> steps 24 to 30, 49 to 51, 55 and 56 of the `## Running order` table in the
...
Steps 25 to 27 ran on 2026-09-05 and 2026-09-06; steps 28, 49 to 51 have their own rows below. The
scope is its own scope because it started from zero while every neighbour was half finished.
...
Seven plans in two halves, with step 26 deciding whether the second half runs.
...
then the curve that sizes one error against the other, then the same nine window positions rerun
...
what step 26 has to return for the second half to start is written into the plans rather than left
to judgement.
...
belongs to step 26, the step everything downstream waits on, and to that plan's review file, and it
...
commute. So what is left at high noise is the non-commutation gap plus the diffused model gap, and
...
3. Measure the correction's size per denoising step against corrector count `k`, on one failing
   pair and one composing pair, and apply a three-way threshold that was written in source before
...
1. **The free bound.** The correction's size at the last denoising step, on the uncorrected path,
   ...
   numerator was recoverable. A size bounded away from zero there is a lower limit on the model's
...
4. **Step 26 returns one of three answers.** Support if, over the last five denoising steps, the
   ratio at the `k` where the curve has stopped changing has fallen by at least
   `MIN_DROP_FOR_SPLIT = 0.20` of its `k=0` value with at least `MIN_REMAINDER_FOR_SPLIT = 0.20`
   still there. Null if the change is under `MAX_DRIFT_FOR_NULL = 0.05` at every step while the
   median displacement is at or above `MIN_CHAIN_DISPLACEMENT = 0.05`. Inconclusive if `k=100` and
...
3. `residual_curves.json` holds its 600 rows (2 pairs × 6 `k` values × 50 steps), the three-way
   threshold has printed one branch with the numbers it was judged against, and the eye read of the
   `k=100` against `k=200` overlay is recorded beside it.
4. The step-26 plan and its review file both carry the sentence that steps 0 to 10 are where the
...
10. Every figure this scope produced passes
    [the figure standard](#the-figure-bar-every-plan-here-is-held-to) and carries a `.json` sidecar
    recording which runs, seeds and settings were drawn.
...
This scope exists as a scope partly so its figures are held to the same standard as the rest of the
...
is, on what scale, for which condition.
...
**A `.json` sidecar sits beside every figure** recording which runs, seeds and settings were
...
failure and correction.** Anything else is prose.
...
sub-scope and not a task, because it is a second corrector family with its own measuring tools.
...
| 25 | [the corrector, and the step size it runs at](plans/tools/02-the-corrector-and-the-step-size-it-runs-at.md) | builds a measuring tool | builds `poe_repair/composers/poe_langevin.py`, proves it inert at `k=0` and with the window past the last step, and fixes the step-size multiplier `c` by search | nothing | ◑ built 2026-09-05, both leak checks byte-identical; the search's pick (`c = 30`) is provisional, since the norm and in-level guards pass step sizes whose samples are noise, and the composing-pair control at step 26 keeps `c = 3` |
| 26 | [what is left once the chain settles](plans/hypothesis/03-what-is-left-once-the-chain-settles.md) | tests the claim | the measurement everything downstream waits on: ‖r_t^(k)‖ and ‖eps_PoE‖ per step against `k ∈ {0,1,5,20,100,200}`, two pairs, one seed, no images and no detector. Applies the three-way threshold | 24, 25 | ❓ ran 2026-09-05 at `c = 30`, `3` and `0.3`: inconclusive each time, cat × dog's `k = 100` and `k = 200` differ by 7% to 38% against the 5% bar; one seed's scatter is the cause, seeds 10 to 12 at `c = 3` are the named next action |
| 27 | [does the corrector compose in the same window](plans/hypothesis/04-does-the-corrector-compose-in-the-same-window.md) | tests the claim | the same nine window positions rerun with the corrector in place of the injected correction, matched to the existing figure | 26 | ✅ ran 2026-09-06 at `c = 3`, `k = 20` against an inconclusive step 26: no window composes (0 of 4 in all ten columns), the sheets read 0 of 8 for the corrector alone, the tail condition and the clean-tail fix are both null; the fidelity lever that moved is the hand-off to the frozen model, which plan 14 of scope 01 carries on |
...
| 55 | [Feynman-Kac steering on a detector reward](plans/baselines/10-feynman-kac-steering-on-a-detector-reward.md) | establishes a baseline | Singhal et al. (arXiv 2501.06848) over the plain PoE score: K particles resampled at five steps on the validated compose scorer's count of the decoded x0-hat, max potential, λ 10; one sheet per pair (cat × dog, butterfly × meadow), seeds 9 to 16, K 4 and 16, against the unweighted control and best of K. Not Skreta et al.'s Feynman-Kac correctors (step 30) | nothing | ⚪ null (W&B `czim1n0w`, done 2026-09-05 20:04): cat × dog 0 of 8 for FK at both K against 0 of 8 for the control, 0 of 128 unweighted K 16 particles composed; step-10 reward agrees with the final verdict on 0.99 of particles, so not blind; harvest pending in the review |
| 56 | [Feynman-Kac steering on top of the rank-32 correction](plans/baselines/13-feynman-kac-steering-on-top-of-the-rank-32-correction.md) | establishes a baseline | plan 10's sampler with the rank-32 step-30050 correction in the proposal at λ 0.5 and 1.2, K 16, seeds 9 to 16; judged at λ 0.5 against the adapter alone, with the headroom stated at 1.2; butterfly × meadow at 1.2 only; ImageReward as the tie-breaker within a count level | 55 | ✅ support at λ 0.5 (Slurm job 50332 on mscluster52, W&B `dfrceppl`, done 2026-09-06 13:00): steering 8 of 8 against 2 of 8 for the unweighted control and 3 of 8 for the correction alone; best of 16 unweighted is also 8 of 8, so the draws compose and the weights choose the picture (mean ImageReward 0.75 against 0.45); at λ 1.2 the control is already 8 of 8 |
...
unit, and the two manual arcs (reading the step-size search table at step 25, judging step 26's
curve) are short enough to live in those plans' Instructions lanes rather than in files of
their own.
...
| The value at any single `k` is a statement about compute budget, not about the problem. Only the trend across `k` is a result, and only once that trend has stopped changing, which is why the grid runs to `k=200` rather than stopping at 100 | step 26, as a threshold and as a caption sentence |
...
Which of the seven plans has to finish before which. The free bound and the corrector build start
today and in parallel; everything else waits on step 26.
...
Step 26 is the only hard stop. A null there does not close the scope: it turns the comparison half
