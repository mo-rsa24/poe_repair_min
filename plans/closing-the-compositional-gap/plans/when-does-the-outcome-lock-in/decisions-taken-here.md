...
noise masks a committed difference until it is removed. The lag is a per-cell measurable and is
the candidate explanation for the 8-to-26-step gap between the correction window and the
divergence numbers.
...
distilled consistency model as the probe on cached states surfaced: none found, never none
exists. Consistency distillation is documented to memorize (arXiv 2604.23552), which the
calibration would catch.

**(e) A cheaper probe from the cache?** Split. Per-step commitment: yes, posterior-mean drift
...
figure scale only, by finish-the-run filmstrips (about an hour per cell). The oracle's unique
value is grid scale: three families, all cells and steps, stability copies and calibration,
about one GPU-hour against tens.

## The decisions

**The read is named the counterfactual finish, never the trajectory's own endpoint.** The
cached run's final frame is the last entry of `latent_trajectory.pt` and needs no oracle. The
oracle answers: stop composing here, let the plain guided model land it. Reverse this only by
finding a question the counterfactual finish does not serve.

**Both prompt sweeps run, each figure labeled by its sweep.** The joint chimera prompt feeds
the per-step commitment probe; the two expert prompts feed the which-mode destination frames.
Cost at today's pool is a few thousand forward passes for both.
...
**Every oracle read carries a stability check.** Perturb the state by 1% of its norm in a
random direction, twice; a flipped endpoint means on-the-fence, a held endpoint means
committed. Chimera states crowd basin boundaries by construction, so the cells of interest are
exactly where reads are least stable; an unchecked committed-looking frame near a boundary is a
coin toss rendered in high fidelity.

**Calibration before use, bars in code.** Roughly 240 states (3 families x 8 pairs x 2 seeds x
5 steps), agreement scored as DINOv2 distance plus scorer-verdict match against the teacher's
finish-the-run ending, at guidance 7.5. Two bars, set in source before results are seen (the
`MIN_MEDIAN_RATIO` pattern): an absolute floor, and a comparability requirement that no family
sits far below another, because a per-family bias corrupts every cross-family figure. On
failure: shrink scope to families and steps that pass, or run finish-the-run everywhere.
...
**Posterior-mean drift rides along as the free cross-check.** Two independent probes agreeing
...
divergence in every cell. Speciation clustering near step 10 explains the gap as
decide-then-descend; speciation inside 18 to 36 kills that story and the diagnostic's premise.
...
**The commitment probes are reads, never causal claims.** The causal side of the correction is
owned by the existing dose-response experiments, and the new figures do not re-claim it.

## Build order, thin end-to-end slices

1. **Basins by hand.** The agreed perturbation check: one cell, steps 5, 25, 40, three endings
   each by finish-the-run with the repo's own DDIM code. No LCM. An afternoon; proves the
   valley picture or kills the proposal.
2. **The free probe.** Posterior-mean drift for a handful of cells from the cache; first
   speciation numbers set beside the divergence numbers; the ordering prediction gets its first
   test. Depends on whether per-step epsilons are cached.
3. **Wire the oracle.** LCM-SDXL download, the adapter (LCMScheduler, timestep mapping, w
   through the embedding, the no-double-guiding assert), one-state smoke against a
   finish-the-run ending.
4. **Calibrate.** The 240-state pass, bars in code, verdict: adopt, shrink, or fall back.
5. **The grid and the figures.** Both sweeps plus stability copies; the per-step counterfactual
   compose-rate curves per family; the three-timestamp aligned figure (correction window,
   speciation, divergence); filmstrips; the PCA overlay beside the outcome curves.
...
- **Finish-the-run at grid scale as the default.** Tens of GPU-hours per full sweep; it stays
  the fallback and the calibration teacher.
- **An h-space probe of commitment.** Needs labels and a trained probe, and the role's
  linear-probe trap applies. Only if the two adopted probes disagree and a third read is needed.
...
- Whether per-step epsilons are cached, which prices slice 2 at zero or at 2 to 4 calls per
  state.
- The numeric values of the two calibration bars and the speciation bar; set in code at slice
