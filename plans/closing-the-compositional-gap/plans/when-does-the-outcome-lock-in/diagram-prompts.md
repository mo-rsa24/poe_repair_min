...
Five pictures of the instrument this scope builds, plus a capstone. Everything here is
`[planned]` and drawn hedged, because nothing exists yet and the map says so truthfully. Every
...
## Abstraction chain
This scope: subject capstone "The whole instrument on one page", process capstone "Five stops
...
side of the parent's cache drum.
...
## Glyph vocabulary
Reused from the parent: cache drum, step strip, window bracket, scorer lens, outcome tile,
researcher persona. Two new glyphs join the family:

| Glyph | Stands for |
|---|---|
| Endpoint spyglass | the distilled jump-to-the-end model (LCM-SDXL) that reads a mid-run state ahead to its settled image; kin to the scorer lens, both are reading instruments |
| Calibration gauge | the agreement dial between the spyglass's ending and the teacher's true finish-the-run ending, with its two bars |

## Reading axes
Dominant axis, left to right: data flow, from a cached state to a filed figure. Secondary axis,
top to bottom: trust, instruments above, gated verdicts and figures below.

## Devices in play
Subject lane: flow colour coding with an in-image legend, truth anchors where a real frame
exists, dashed hedged outlines on every platform while pieces are `[planned]`.
...
Scene: left, the cache drum with three thin trays labeled by family; one state card slides out.
...
connected by a dashed arrow from the spyglass, with a small tag on each arrow naming its sweep.

Cast: cache drum (per-step latents saved for later analysis), the endpoint spyglass (the
...
"endpoint spyglass (distilled)", "joint sweep: does it compose from here", "expert sweep: which
animal wins", "settled frame, not a blur".
```

### Prompt 3 (Subject): The free probes
...
into three copies (one nudged up, one untouched, one nudged down), each walking a short step
strip to its own small outcome tile; a bracket compares the three tiles. Right panel "the
posterior mean settles": a step strip with a small blurred thumbnail above each early step
sharpening toward the late steps, and a curve beneath that descends and flattens, with a marker
where it flattens.

Cast: the state card and its two nudged copies, three short step strips (finishing the run with
...
Text in the image: title banner "two probes that need no new model". Labels: "nudge, finish,
...
Scene: centre, a large dashed gauge glyph with two marked bars on its dial. Feeding it from the
...
cards, the two bars (an absolute floor, and no-family-far-below-another), per-family status
chips.
...
"teacher ending", "bar one: agreement floor", "bar two: families comparable", "verdict: adopt,
shrink, or fall back".
```
...
Scene: one long step strip, noise at the left, finished image at the right, on a dashed
...
Cast: the step strip, the window bracket (when the correction is allowed to act), the
...
### Subject capstone: The whole instrument on one page
...
Scene: left to right on dashed platforms. The cache drum with its three family trays releases a
state card. It forks: up to the endpoint spyglass, down to a finish-the-run teacher panel (a
short step strip ending in a tile). Both endings meet at the calibration gauge with its two
bars. Past the gauge, a scorer lens reads the settled frames, feeding a wide results panel on
...
register board receives one dashed line from the results panel.

Cast: cache drum, state card, endpoint spyglass, teacher step strip, calibration gauge, scorer
lens, three-timestamp strip, per-family curves, valley fork vignette, register board.
...
passed is the only way through; thin gray from results to register board. Legend lower left.

Text in the image: title banner "when does the outcome lock in". Labels: "cached steps, three
families", "read the ending early", "true ending, walked", "agreement, two bars", "count the
animals", "three timestamps", "to the paper's figure slots".
```
