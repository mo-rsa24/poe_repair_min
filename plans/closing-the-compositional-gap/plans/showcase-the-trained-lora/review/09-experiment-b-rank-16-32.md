...
Run kind: ablation over the rank axis (one question per rank rung).
...
| File | What it holds |
|---|---|
| [design](../plans/09-experiment-b-rank-16-32.md) | the two launches, the matched-steps rule, the null bar |
| this file | the verdict per rank, read beside the oracle-ceiling panel |
...
- **The oracle-ceiling panel**: plan 12's adapter-vs-true-correction side-by-side; the key that
  says whether a capacity null is a dead end or the answer.
...
- [ ] ⚠️ **At matched 100k, do rank 16 and rank 32 sit inside rank 8's held-out seed-noise
  band?** Bar fixed at launch: both inside is a capacity null; either outside by more than the
  band reopens capacity as the lever, with plan 12's panel deciding how hard to push it.

## Written before the run, answered after

Navigation: ⬅️ [The bar](#the-pre-registered-bar) | 📋 [TOC](#table-of-contents) | [Next](#could-the-answer-be-an-artefact) ➡️

- [ ] ⚠️ One question per rung, as an ablation owes: does r16 differ from r8 anywhere in the
  tracking set's reads? Does r32 differ from r16?
- [ ] ⚠️ Does the crispness read move with rank even where compose rate does not?
...
- [ ] ⚠️ **Was the instrument sound?** Same frozen manifest hash on all three runs' configs.
