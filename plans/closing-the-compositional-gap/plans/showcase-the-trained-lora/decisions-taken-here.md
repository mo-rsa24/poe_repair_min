...
q/k/v, 100k optimizer steps (2,000 epochs) over 88 cells (11 pairs x 8 seeds), W&B project
...
training pairs carry 22.6% of the corrections' energy (norm-matched chance floor 16.1%, iid floor
2.1%); the same directions carry 7.4% of six unseen pairs' energy (random-direction line k/d =
...
- A structure figure plots energy-at-k (y, 0 to 1) against k (x): curves train, held-out
  projection, and the applicable chance anchor; self-fit floors guard the train curve, k/d guards
  a projection. Never a self-fit floor against a projection (that mistake was made live in a
  walk and is the reason the rule is written down).
- No MDS or UMAP panel may invite the reader to read distances.
- Dose figures: compose rate (y, 0 to 1) against lambda (x), one curve per injected vector, the
...
seeds share one noise-shell radius and seed character can only be directional. A per-cell x_t
trajectory plane is nearly lossless (top-2 variance 99.5% on the measured cell) but a plane
over r_t or eps_PoE hides roughly 45% of the variance and its caption says so.
...
8 pairs x 9 windows x 4 seeds, 288 scored cells): compose rate by window centre falls 0.656
...
commitment and its position is shared while commitment varies. A sharp-edge width-1 sweep is
...
Optional descriptive extra, not owed: the repair cell's appearance strip (decoded predicted-x0
...
-0.174 over steps 20 to 49, measured on cat x dog seed 9; recompute over the 64 showcase cells
if drawn).
...
the scorer-invisible softness and B's rank-ablation figure. The free plateau read runs first
and is their interpretation key: `eval/frac_distance_reached` plateaus near 0.4
(instrument-02); flat across epochs and checkpoints means ceiling, still rising means waypoint.
...
steps each, everything else identical (`sweep_s1_rank.sh` is the capacity harness). Buys the
rank-ablation figure: same cells, same lambda, same inference across r8/r16/r32 at matched
step counts. Null bar: r16 and r32 held-out compose rates within r8's seed-noise band. The
oracle-ceiling panel is B's interpretation key: oracle crisp with adapter soft is the one
outcome that makes capacity the regime-correct lever.
...
lets any discovered direction use causal language, and it shares one harness with the
adapter-dose sweep below.

**Two tasks owed alongside, already routed to figure-01-the-transfer-figures.md:** score the
unscored 70k to 100k per-epoch samples (closing F8a's gap), and the qualitative
oracle-ceiling panel (adapter-corrected beside true-r_t-injected renders, same held-out
cells, caption stating the comparison is qualitative).
...
3) and files captured panels into that runbook page's screenshot slots.
...
before launch, rendered every 10k steps, logged to W&B. Contents: the four F9 cells
(held-out pairs, seed 9), the repair cell cat x dog seed 1, compose rate over the held-out
pool (8 pairs x 8 seeds), direction-cosine and fraction-of-distance-reached (already wired),
...
the missing animal), as one follower process: it watches the run's checkpoints folder, and
...
cells; storage forced to pooled activations or a cell subset; each read ends in its
intervention (inject the found direction before about step 10, score composition), because a
direction figure without an intervention is the correlational-picture trap. Full
residual-free steering sweeps are parked as their own idea thread, not this paper. The
paper's figure ladder is oracle-lean, so this plan is a deliberate scope addition. Reverses
if: the interventions fail to move compose rate, in which case the figures stay out of the
main text.
...
with large ||r-hat||. Preflight before reading anything: verify PoE(A,A) reduces to Mono(A)
within fp16 drift (copy the `run_cfg_masked(all_off)` identity pattern). Guidance is never raised
to buy crispness (off-manifold trap).
...
reproducibility diagnostic (two LoRAs on disjoint seed subsets, same eval noise, same corrected
image?) is deliberately optional: one extra training, revisit after the probe lands.
...
controls, same machinery as the oracle sweep with the injection source swapped. The dog x dog
probe is this sweep's zero-interaction control cell; Experiment C above is its lambda grid; the
three share one harness.
...
(joint prompt, plain PoE, adapter), same cells and seeds, with the repair-cell strip beside
it as the anecdote; (iii) wording rule: "restores composition the joint prompt loses on this
pool", never "outperforms SDXL" or "outperforms large models"; (iv) guidance stays fixed at
7.5 and stated in every caption; no guidance sweep enters unless the spine ever claims why
the joint prompt mode-drops (it does not today).

## The seed-character panel

A free read from already-scored cells: per-seed marginal compose rate across pairs, binomial
noise as the bar, the bar in code. If some seeds compose across pairs while others never do,
starting directions carry a pair-independent character and the basin framing gains a measured
panel at zero GPU cost; if the spread sits inside binomial noise, the panel reports that null
and the seed-character idea closes.
...
final latents (cache-only, an afternoon, no sampling). Running it does not gate any figure; it
upgrades "2,000 epochs over 88 cells is deep in memorisation territory" from a plausibility
argument to a placement.

## Still open, on purpose

The 50k-versus-100k softness sanity read (free, gates nothing). The oracle-ceiling panel's
outcome, which decides whether B's result is read as a capacity finding or a dead lever. The
non-animal scorer re-validation, which opens tier-three captions.
