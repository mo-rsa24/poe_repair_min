...
```
/sync-plan-tree @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md — <what the lambda=1 column classified>
```

## Recommended skill

▶ `/design-figure` ✅ two grids on a shared axis, with rows that deliver different absolute amounts
   and cells that cannot reach `λ=1`, is a layout decision that has to be made before the render,
...
| 30 | [idea-01: feynman-kac-correctors-gated](idea-01-feynman-kac-correctors-gated.md) ⚠️ | gated on this and on the gate at step 26 |

Design only. Verdicts and run state live in
[the paired review file](../review/baseline-02-three-rules-on-one-dose-axis.md).
...
four rows by five columns, `λ ∈ {0, 0.25, 0.5, 0.75, 1}` at seed 9. Thirty-six cells in total.

**Associated materials.** [The review questions](../review/baseline-02-three-rules-on-one-dose-axis.md),
...
**The framing depends on the gate, even though the runs do not.**

Under a split at [step 26](hypothesis-02-what-is-left-once-the-chain-settles.md), these grids
illustrate a diagnosis. Under a null they are a baselines table. Same cells, different sentences,
...
**Dependent variable.** Whether the cell composes, by the detector and by eye, and whether the row
reaches the joint render at `λ=1`.

**Falsify condition.** This is a baseline and a figure, so there is no bar that closes the scope.
...
not the gate came back with a split.
...
have received more.
[The timing plan in the causal scope](../../closing-the-compositional-gap/plans/does-the-correction-cause-composition/plans/hypothesis-03-when-in-the-run-it-matters.md)
met exactly this and answered it with a `--mode matched` arm, implemented at
...
at a point the joint model never visits does not land on the joint render. Those cells are labelled
not-an-identity, on the cell, so a reader does not read them as failures. This is what makes the
...
the cells no rule can reach at `λ=1` are labelled rather than left looking like failures.

**Goals**

1. The generalised injection implemented and verified on one cell per row.
...
- [ ] **0.1** Check this plan conforms and its instructions are concrete, before acting on it.
  - Paste: `/verify-plan @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md`
...
- [ ] **0.2** Confirm all four rows can produce a per-step prediction before any grid is planned.
  - Plain product-of-experts from [poe.py](../../../poe_repair/composers/poe.py), SuperDiff from
    [step 28's hook](baseline-01-superdiff-at-this-repos-fifty-steps.md), and both corrector rows
    from `poe_repair/composers/poe_langevin.py`.
  - **Done when:** `r_t^M` is formed on one cell for each of the four rows and its per-step norm
...
◀ **Needs: [task 0.2](#0--preflight-check-this-plan-before-working-from-it)**, so every row has a
...
- [ ] **1.1** Implement the injection: for rule `M`, inject `eps_M + λ·r_t^M` where
  ...
  - **Done when:** at `λ=0` each row is byte-identical to that rule run alone, checked the same way
    [step 25](instrument-01-the-corrector-and-the-step-size-it-runs-at.md) checks its composer.
...
- [ ] **1.3** Decide and record how the different absolute amounts per row are handled.
  - Either put the delivered absolute total on each row's label, or run the `--mode matched` arm
    from [interaction_term_dose_matched.py](../../../scripts/interaction_term_dose_matched.py),
    which [the timing plan](../../closing-the-compositional-gap/plans/does-the-correction-cause-composition/plans/hypothesis-03-when-in-the-run-it-matters.md)
    already used for this exact problem.
  - **Done when:** the choice and its reason are written into the review file **before** any grid
    renders, since it changes what the cells are.
...
because it changes what every cell is.
...
- [ ] **2.1** Grid one: columns are seeds 9 to 12, `λ` fixed at 0.75, four rows. Render the same
  ...
  - **Done when:** 32 cells exist (4 rows × 4 seeds × 2 `λ` values), counted.
- [ ] **2.2** Grid two: columns are `λ ∈ {0, 0.25, 0.5, 0.75, 1}`, seed fixed at 9, four rows.
  ...
  - **Done when:** 20 cells exist, counted.
- [ ] **2.3** Label the cells the corrector rows cannot reach at `λ=1` as not-an-identity, on the
      cell rather than in a legend or a caption.
  - **Done when:** the two corrector rows' `λ=1` cells carry the label in the rendered figure.
...
- [ ] **Capture the failures this plan hit**, while they are still fresh.
  - Paste: `/ingest-error-pattern --from-run-log @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md`
...
- [ ] **Bring the tree current** with what actually happened.
  - Paste: `/sync-plan-tree @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md — <one line>`
...
- [ ] **3.2** For grid one, count composed cells per row by eye and compare against the detector's
      count.
  - [The timing verdict](../../closing-the-compositional-gap/plans/does-the-correction-cause-composition/review/hypothesis-03-when-in-the-run-it-matters.md)
...
- [ ] **3.3** Write both counts into the review file, side by side, with the disagreements named
      cell by cell.
  - A summary count hides exactly the cells a reviewer would ask about.

▶ **Next: [the close out](#close-out--record-what-this-plan-taught)**, then
[step 30](idea-01-feynman-kac-correctors-gated.md), which is gated on this.
...
> whose unreachable cells look like failures, argues against itself.
...
- 32 cells in grid one, 20 in grid two.
...
- Instruction 3.3 has recorded both counts with the disagreements named cell by cell.
...
[the review file](../review/baseline-02-three-rules-on-one-dose-axis.md).
...
|---|---|---|---|---|---|
| `across-composition-rules/rules-at-one-dose.png` | — | rows are the four composition rules, columns are seeds 9 to 12 at `λ=0.75`, plus the same grid at `λ=0`. Cells are the final generated picture, green border where the detector scored two separate animals | task 2.4 | ⏳ | **Supplementary, not main text.** It varies the seed, which the paper already establishes elsewhere. Sidecar records rows, seeds, `λ`, the corrector rows' `k` and `c`, and the delivered absolute amount per row |
| `across-composition-rules/rules-as-the-dose-rises.png` | — | rows are the four rules, columns are `λ ∈ {0, 0.25, 0.5, 0.75, 1}` at seed 9. The `λ=1` cells the corrector rows cannot reach are labelled not-an-identity on the cell | task 2.4 | ⏳ | **Main text, if the gate at step 26 passed.** The `λ=1` column separates rules that combine scores from rules that do something else, which is a statement about the dynamics rather than a leaderboard |

**Two sentences these captions owe.** `λ` is a fraction of each row's own correction, so rows
deliver different absolute amounts unless the matched arm was run; whichever was chosen is named.
And the not-an-identity cells are expected rather than failed, because the chain has already moved
...
|---|---|
| the `λ=1` classification | the review file, then [step 30](idea-01-feynman-kac-correctors-gated.md), whose gate reads it |
...
|------|---------|--------------|---------|
| Check the plan | `/verify-plan @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md` | **task 0.1**, before any work | Conformance and thin instructions reported |
| Capture patterns | `/ingest-error-pattern --from-run-log @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md` | **the close out**, after any red run | Errors added to catalogs |
...
| Bring the tree current | `/sync-plan-tree @plans/is-the-gap-the-samplers-or-the-models/plans/baseline-02-three-rules-on-one-dose-axis.md` | **the close out** | Statuses, running order and Error Matrix match reality |
...
| Path | Why it is read |
|---|---|
| [scripts/interaction_term_dose_matched.py](../../../scripts/interaction_term_dose_matched.py) | the `--mode matched` arm, which already answered the different-absolute-amounts problem once for this project |
...
| `poe_repair/composers/superdiff.py` | the SuperDiff row and its per-step prediction, from [step 28](baseline-01-superdiff-at-this-repos-fifty-steps.md) |
...
[Step 30, Feynman-Kac correctors, gated](idea-01-feynman-kac-correctors-gated.md). It runs only if
the corrector arm moved something; otherwise it closes unrun with the reason recorded.
...
run the `--mode matched` arm.

#### 🟡 not-an-identity cells read as failures

**When it happens:** the corrector rows at `λ=1`.
**What you see:** two cells that do not match the joint render, in a column where the other two do.
**Why:** the chain has already moved the latent off the joint trajectory.
**How to fix:** label the cells on the cell, per task 2.3, and say in the caption that it is
