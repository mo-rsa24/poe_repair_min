#!/usr/bin/env python
"""Cat and dog, one held-out seed, rendered once by every adapter this project has trained.
...
the first. The shell loop below calls --render once per row, then --sheet assembles.
    ...
    for i in $(seq 0 23); do python scripts/across_adapters.py --seed 9 --render $i; done
    python scripts/across_adapters.py --seed 9 --sheet

One sheet per seed. A checkpoint here can be crisp on one seed and an engraving on the next, so a
single seed decides nothing and three is the smallest honest read.
"""
...
SEED = 1                             # replaced by --seed
WORK = O / "across_adapters_seed01"  # replaced by --seed
...
ROWS = [
    ...
    ("43 cells, first 10, on the picture", "showcase/v57w10/v57w10-02-picture",       7000,  32),
]
...
def sheet() -> None:
    ...
    out = REPO / f"across_every_adapter_seed{SEED:02d}.png"
    canvas.save(out)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--seed", type=int, default=1,
                    help="held-out seed to render every adapter on. Each seed writes its own work "
                         "directory and its own sheet, so seeds never overwrite each other.")
    ap.add_argument("--render", type=int)
    ap.add_argument("--sheet", action="store_true")
    ap.add_argument("--list", action="store_true")
    a = ap.parse_args()
    SEED = a.seed
    WORK = O / f"across_adapters_seed{SEED:02d}"
