...
to row 5, which corrects the whole run. Each row pairs the real sampled
image with a track of the 50 denoising steps: green where the correction was
on, grey where it was plain PoE. The track is the same drawing
scripts/window_strip.py already uses for F4a's sliding window, so a reader
who has seen one has seen the vocabulary for both.

The point of drawing it this way rather than as a seed x cutoff grid (an
earlier attempt at this slot) is to make the *manipulation* legible on the
page, not just its outcome: a reader can see which steps were corrected
without reading the caption.

Single seed by design, so the picture reads as "watch this cutoff grow", not
as a population claim; the pooled compose-rate curve across all 8 pairs is
...
FIG_NAME = "F4g-more-start-same-ceiling"
...
def main() -> int:
    ...
    for i, win in enumerate(windows):
        ...
        for s in ax.spines.values():
            s.set_visible(False)
        ax.set_ylabel(f"0–{win[1]}", fontsize=8, family="serif", color=INK,
        ...
        tr.add_patch(Rectangle((win[0], 0.2), win[1] - win[0], 0.6,
    ...
    fig.suptitle(f"seed {seed}, cat × dog\ngreen = correction on, grey = plain PoE",
