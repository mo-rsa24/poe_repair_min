#!/usr/bin/env bash
# V6B: V6 with both concept branches frozen. Loss z - (a_base + b_base - u): only the empty
# branch is adapted. Same second-forward rule as V6A, for the same reason. Expected to fail
# the way V3 does: one shared tensor cannot carry a per-pair correction.
...
export RUN="${RUN:-v6b_render_teacher_only_null_r16_s0_25}"
...
export EXTRA="--render-teacher --adapt-null-only"
