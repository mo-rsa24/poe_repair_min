...
| 66 | 08-improve-the-rank-32-pooled-adapter/tools-01-the-three-switches-and-the-guidance-interval | `--exclude-cells`, `--train-step-range` and `--orth-weight` in the pooled trainer, a guidance interval in the windowed sampler, the 14-pair pool and the exclusion list; each switch prints the count it selected and the interval is held against the kept render at 6 grey levels | ⚠️ |  |
| 67 | 08-improve-the-rank-32-pooled-adapter/tools-02-the-blind-label-pass | the read this scope is judged by: strips of four tiles per seed, ids instead of condition names, the three labels (clean, unclear, not two), and the count and both-names reads logged beside them for ordering only; proved on examples whose labels are already settled | ⚠️ | 66 |
| 68 | 08-improve-the-rank-32-pooled-adapter/baselines-03-what-the-stacked-sampler-alone-does | checkpoint 30050 rendered shipped and stacked over 17 cat × dog and 8 elephant × penguin seeds and labelled blind, asking whether changing how the checkpoint is run already gives more clean seeds; bars `MIN_CLEAN_GAIN` and `MAX_CLEAN_LOST` in source; fixes which column the training runs are read against | ⚠️ | 66, 67 |
| 69 | 08-improve-the-rank-32-pooled-adapter/hypothesis-04-the-parent-and-the-four-children | one rank-32 run with five changes on at once (14 pairs, wrong-target cells dropped, training steps 0 to 24, the out-of-span error charged three times, weight decay with an averaged copy of the weights) and four children each undoing one, chained overnight on one Blackwell, all rendered both ways and labelled blind against the baseline | ⚠️ | 66, 67 to launch; 68 to read |
| 70 | 08-improve-the-rank-32-pooled-adapter/figures-05-the-comparison-sheet | two sheets (joint target, plain PoE, 30050, best run) over all 25 seeds with every blind label under its tile, filed under `artifacts/results/` with its card, and the correction's size per pair reported beside them claiming nothing until labelled examples are laid against it | ⚠️ | 68, 69 |
| 71 | 09-designing-the-correction-loss/tools-01-the-three-instrument-fixes | the undialled error `‖(a+b−u) − ε̄_J‖²` logged beside the loss, a checkpoint written at every render instead of every fourth one, and every objective-changing flag written into `config.json`; each proved by a two-epoch smoke. Its ratio against the fit loss is the cheapest falsification of the whole variation programme | ⚠️ |  |
| 72 | 09-designing-the-correction-loss/reading-02-the-v0-foundation | six pillar entries for terms `context/` never defined, the thirteen scattered probe json collected into one file, a chart script that can redraw all eleven figures (the one that drew the existing eight is not in the repo), the seed-noise band computed from the definition pre-registered in scope 01, the rank-ablation figure nobody made, and `00`'s report page: the shape every later variation copies | ⚠️ | 71 |
| 73 | 09-designing-the-correction-loss/hypothesis-03-the-v0a-read | the four matched rank-16 experiments that finished 2026-09-12 and are recorded nowhere, scored on eight held-out seeds and read blind against a mechanism bar and an outcome bar; `00a`'s page and the prompt-rewrite finding. Trains nothing | ⚠️ | 72 for the page shape |
| 74 | 09-designing-the-correction-loss/hypothesis-04-freezing-the-empty-branch | the check that training and sampling agree about what is frozen, then reading the empty branch from cache and narrowing the batch to `2K`, then 50,000 steps read against `00` at matched steps. Can falsify the derivation that variations 02, 03 and 04 all rest on | ⚠️ | 71 for the number, 73 for the approximate fix it is read against |
| 75 | 09-designing-the-correction-loss/hypothesis-05-training-on-the-renders-that-composed | variation `06`: a second data path that encodes a render a person judged to show both concepts, noises it, and trains the composition against the noise that was added; gated by a scaling curve over 10, 20, 40 and 72 nested cells with `MIN_SCALING_SLOPE` and `MIN_COMPOSE_AT_FULL` in source. A flat curve ends the variation without any comparison against `01` being read | ⚠️ | 74, because it moves the target and the empty branch against `00` at once |
| 76 | 09-designing-the-correction-loss/hypothesis-06-fitting-the-whole-path-and-handing-back-the-tail | variation `08`, the only plan in the scope aimed at fidelity rather than composition: measure how much of the correction the adapter actually applies per denoising step, then sweep the step after which the adapter is detached and the frozen model draws the rest, on checkpoints that already exist. Trains nothing unless that sweep falls short | ⚠️ | nothing; its first two stages read existing checkpoints |
...
| `08-improve-the-rank-32-pooled-adapter/` | live, nothing started | the better checkpoint: the rank-32 pooled adapter trained again with five changes on at once and four children that each undo one, judged by eye against checkpoint 30050 under two samplers. Last because it replaces the checkpoint scope 01's figures are measured from, and only if it wins |
...
- The mention of **the root running order** in [the progress map](plans/progress-map.md), which draws the 76 steps of `## The paper: what has to land` as a rail and pins the current position on it.
