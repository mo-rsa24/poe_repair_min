...
One row per item. A folder whose name starts with a figure id (F2, F6, F7, F8) feeds that row of
...
| Item | Figure it feeds | State |
|---|---|---|
| `paper/iclr/figures/compose-rate-as-correction-rises.png` | F2, the headline figure: compose rate as more correction is added | result stands (the true correction takes compose rate from 7% to 93%, the controls stay flat); the exact percentages wait on the re-score; **caption owed** |
...
| `artifacts/results/which-way-the-correction-points/what-the-spectrum-measures/` (`QUERY.md`, `result.json`) | F6, why it is learnable | **what the spectrum would give by luck does not control for ‖r_t‖ spread, and against one that does the pooled stack is 1.4x at k=8 rather than 10.7x.** What direction structure survives is within single runs (4.8x at k=8), not across pairs (1.2x). F6's shared-structure argument does not stand and that figure needs a decision |
| `artifacts/results/residual-dynamics/content-change-relative-to-attention-change/` (`measure-fairness.md`, `RUN_ON_THIS_NODE.md`) | F7, the mechanism panel | replicated, median 1.52x over 64 runs. `measure-fairness.md` is why the obvious measure gives the opposite answer, which the appendix needs; **caption owed** |
...
| `artifacts/results/when-the-correction-must-arrive/commitment-step/` (`QUERY.md`, `result.json`, `commitment-step-per-pair.png`) | no figure yet, EXP-01 of `report/experiments-log.md` | the step where a run's estimate settles varies by pair (medians 18 to 36 over 162 runs), but every pair settles after the correction has stopped working. EXP-04 then showed it predicts nothing about the window, so this measure is not tracking the decision |
| `artifacts/results/when-the-correction-must-arrive/window-vs-commitment/` (`QUERY.md`, `result.json`, `window-vs-commitment.png`) | no figure yet, EXP-04 of `report/experiments-log.md` | **all 8 pairs peak at the same window, steps 0 to 10, while their settling steps span 13.** The window does not move with the pair, so the adapter's schedule cannot be misplaced per pair and composition is decided inside the first fifth of the run. Left-censored: the best window is the earliest the grid holds |

> Held-out means kept out of training and used only for testing, so a number measured on a held-out
> pair says something about pairs the method never saw.
...
| `report/instrument_smoke.md`, `report/normalization_preregistration.md` | how the measuring tools were built and checked, and what was committed before any result was read |
| `report/decision-timeline.md`, `report/RESULTS_SUMMARY.md` | the running record; RESULTS_SUMMARY defers to the review files where they disagree |
...
| `artifacts/plans/parked/artifact-reconciliation/inventory/` | the artifact-reconciliation scope's four output documents |
