...
The account, in order, of what each objective was, what ran, what it said, and the decision it forced. Downstream plans and write-ups backfill against this record.
...
> Mono-free means the method never gets to encode the combined phrase "a cat and a dog". It only ever sees the two separate phrases, so anything it restores has to come from what it learned about the correction.

**The pyramid (deployment-unit question at each tier).** Overfit → Survive-Noise → Cross-Pair → Group-Wise → Scale. Each tier widens the set of pairs and seeds kept out of training, and asks what the reusable unit is: one pair at one seed, a pair, a group, or one LoRA for the whole taxonomy.

> Held-out means kept out of training and used only for testing. A result on a held-out pair or seed says the method reached something it never saw.

## How to read this log

- Each entry: **Question / Ran / Result / Decision / Fired next**, tagged `[tier · date · status]`.
- **Status:** ✅ done · ◑ partial/in-progress · ⏸ trained-not-evaluated · ⚠️ open · ❎ negative result (kept as evidence).
- **Dates:** firm from 2026-05-14 on (checkpoint/cache mtimes + git). The early-May research questions (G01–G03) are `~approx`: their artifacts are regenerable and not in git, so the dates are reconstructed from the cache-build era and are confidence-low. The operations entries are firm (2026-07-21).
...
- **Question.** Is the gap between broken PoE and the Mono ceiling fixable *in principle*, meaning can you walk PoE → Mono from the PoE trajectory?
- **Ran.** The true correction: compute `r_t = ε̃_Mono − ε̃_PoE` from the joint-prompt prediction and inject it during sampling, to find out whether the walk is real (`plans/01-veracity.md`).
- **Result.** Injecting that true residual walks PoE → Mono in image space. The repair problem is well-posed. *(Date approximate; artifacts regenerable, not in git.)*
...
- **Question.** What is `r_t`'s shape, timing, and locus, meaning what would a learner have to predict?
...
- **Result.** `r_t` is structured rather than noise, and it has a commitment window in the trajectory. That defines the target and the magnitude curve later used to sanity-check the LoRA.
...
- **Question.** With CFG masked at various step ranges on clean SDXL (no LoRA, no Mono), how much composition is already reachable? This is the baseline with no correction at all.
- **Ran.** A series of conditioning-window runs turning CFG on and off over different step ranges, cat×dog seed 42 (`plans/03-conditioning-window-baseline.md`; code restored under `conditioning_window` at git C13, 2026-05-18).
- **Result.** A minimum conditioning window still yields a recognisable cat+dog. That establishes the baseline the LoRA's marginal effect is measured against (same x_T reused).
- **Decision.** Baseline established. Any LoRA claim must beat this, not just beat raw PoE.
- **Fired next.** G04.
...
- **Result.** λ=0 is byte-identical to vanilla PoE, which is the check that has to pass before anything else counts. λ=1 morphs the chimera into two distinct animals by ~epoch 500–600. Masked sampler at `all_on` ≡ standard sampler at λ=1 (Δ=0). Reaches ~40% of the PoE→Mono latent distance and is still moving. **The deployable result exists on the one pair and seed it was trained on.**
...
- **Question.** Can the residual be predicted *outside* the UNet (external correctors), or by forces computed inside PoE that never see the joint prompt, instead of by a LoRA?
- **Ran.** Group-A: latent-CNN, latent-UNet, frozen-feature-MLP external correctors (`plans/05-`artifacts/_quarantine/results-archive/group-a-failure.md`, W&B `poe-repair-group-a`, three runs finished). Internal-force: attention-overlap repulsion + score-alignment damping (`plans/06-internal-force-failure.md`).
- **Result.** All five fail to close the gap. The training runs completed; the *finding* is failure.
- **Decision.** The LoRA result is meaningful, since no other corrector achieves it. Report the failures negatively as the contrast set, and scale the LoRA rather than the alternatives.
...
- **Decision.** Pooling might only recover a weak seed-mean. Don't assume it works, and test pooling empirically before believing cross-seed generalisation.
- **Fired next.** G07.
...
- **Fired next.** Feeds G10/G11 but is not yet a question that closed.

## G10: Is "group" a deployable pooling unit? [Group-Wise · 2026-05-27 · ◑ one short g6 run only]

- **Question.** Train one LoRA on 7 within-group pairs × all 12 seeds, hold out 3 pairs of the same group (pair axis only). Does it compose on held-out pairs, beating Plan-10's short single-pair sibling run?
- **Ran.** `cross_pair_lora_pooling` within-group, group **g6 only** (`within_group/g6/main`, trained ~30k steps; mid-training evaluation over the trained-against-tested grid, `eval_crossbar/step_020000`, `in_in`+`out_in`, manifest `n_cells_planned = n_cells_sampled = 43`, 43 jsonl rows) (`plans/16-within-group-cross-pair-cross-seed.md`).
- **Result.** **One short g6 run only.** g1–g4 have YAMLs but no run dirs. The mid-training eval produced 43 of 43 runs; the full sample→contact-sheet→Task-D tail did not run.
- **Decision.** This tier is not read: the g6 run proves the wiring, not the per-group claim. "Group is a deployable unit" remains untested for 4 of 5 groups.
- **Fired next.** Disambiguates G11 only once g1 (+ others) run.
- **Evidence.** `artifacts/rung3-group-wise/cross_pair/within_group/g6/main__wandb-ow1jo0xq/` (ckpt step 30000 loads; 43-run grid verified).

## G11: Can ONE LoRA span the taxonomy, held out on both axes? [Scale · 2026-05-26 · ⏸ trained-not-evaluated]

- **Question.** Train one rank-8 LoRA on 5 representative pairs × 8 seeds (40 runs); evaluate the four-quadrant grid of trained-on and unseen pairs against trained-on and unseen seeds. Headline quadrant: held-pair × held-seed (`out_out`).
- **Ran.** `cross_pair_lora_pooling/all_groups/main` (W&B main `2em6frqv`), per-step multi-pair trainer, trained to `lora_step_030000.pt` (`plans/11`, `plans/15-cross-pair-cross-seed-lora-mscluster.md`). A later run `0y9un0o4` **died early**, and it is the one noise run that changed direction (below).
- **Result.** **Trained, never evaluated on the grid.** `samples/` holds only `per_epoch/`; **no `cells.jsonl`** → `sample_crossbar` / `contact_sheet` / `task_d_bridge` never ran on it. The strongest quadrant (`out_out`) was **never sampled anywhere**. The checkpoint itself loads (bytes intact).
- **Decision.** **RE-RUN (finish training, then run the grid evaluation).** The taxonomy-spanning "one LoRA" claim, the strongest form of the deployable contribution, **does not yet exist as a result**. The died-early `0y9un0o4` is load-bearing precisely because it left this state: trained-but-unevaluated.
...
- **Ran.** `/data-inventory` plus a classified pass over both roots (`inventory/01-artifact-inventory.md`, `inventory/02-two-root-classified.md`, `inventory/scripts/01_inventory.py`).
- **Result.** 22G / 13,214 files; 4 W&B projects (`poe-repair-{lora,cross-seed,cross-pair,group-a}`). Flagged: 3 empty-looking `lora/` pair dirs, died-early (`0y9un0o4`, `ow1jo0xq`), false-starts (`lu7g7svh`, `9ux1sm67`), benign sync-fatal (`d5b2706v`). Heavy seed banks live on `/datasets`, light eval samples in repo (same pair name at a different stage, so they are not duplicates).
...
- **Result.** **16/16 checkpoints load** (420 LoRA keys, shape (8,640)), including all four suspects (bytes intact; suspect = run completeness, not corruption; `d5b2706v` intact at step 16510). Caches: 645 runs; 24 "1-shard" runs are **by-design eval stubs** (`build_eval_cache.py`), not truncation. Top-level manifests **stale** (declare 3 runs vs 645). `all_groups/main` **never evaluated on the grid** (confirms G11). `manifold_cache` **0/7** (confirms G12). **Correction (supersedes OPS-01's "0-byte stubs" wording):** the three `lora/` stubs are **live symlink views** to cross_seed banks (kept); only `training_cache_overfit_catdog` is a genuinely-broken view (quarantined). `du`/`find -type f` had read symlink-only dirs as 0 bytes.
- **Decision.** **Keep** most; **RE-RUN** the `all_groups` grid evaluation and `manifold_cache`; **regenerate** stale manifests; **quarantine** discards (false-start, dryrun, superseded, broken view). Read-only: recommend, don't delete.
- **Fired next.** OPS-03.

## OPS-03: Make the artifact tree navigable + canonical [✅]

- **Question.** One naming and directory scheme keyed by stage, then experiment, then pair, then seed, so both roots are navigable without breaking references?
...
- **Decision.** Canonical layout established. Spawned the **`artifact-reconciliation` sub-scope** (`plans/standing/artifact-reconciliation/`) with 01–04 as done tasks + a standing "re-sweep on new runs" node, so this stays current.
- **Fired next.** The standing re-scan on new runs (`plans/standing/artifact-reconciliation/plans/05-resweep-on-new-runs.md`).

---

## Coverage manifest (what this record does NOT yet cover)

Honest gaps; do not read the log as "everything landed":

- **Open re-runs forced but not done:** the G11 `all_groups` grid evaluation (`out_out` never sampled); the G12 `manifold_cache` backfill (0/7).
- **In-progress tiers with no verdict:** G09 (per-group cross-seed, G1–G4 part-trained, no verdicts); G10 (within-group, one short g6 run only, g1–g4 not started); G08 (G1–G3 single-seed not trained, MDS not run on G4).
- **Quantitative claims without a regenerated real table here:** the G04 "~40% of PoE→Mono distance" and "two distinct animals by ep500–600" are eyeball/plan-stated, not re-verified with a metric table in this log. Checking them with GroundingDINO or VQAScore was deferred in Plan 04. If a supervisor needs the number, that exhibit is `pending`.
- **Dates G01–G03 are approximate** (regenerable artifacts, not in git), so confidence is low; firm dates begin at G04 (2026-05-14).
- **W&B not queried live**, so statuses are inherited from local run-dir reconciliation (OPS-01).
...
## OPS-04: Where does output that is not part of the five tiers go? [✅ decided, ⚠️ move pending]

- **Question.** Eight experiment top-dirs sit outside the canonical `artifacts/` scheme. Do the diagnostics and mechanism-study dirs belong under `rung1`–`rung4`, or somewhere else?
- **Ran.** A read-only re-scan (`inventory/sweeps/2026-08-04-resweep-detection.md`) → scope call + load tests (`inventory/sweeps/2026-08-04-scope-call.md`) → `04_apply_layout.sh` extended, dry-run only.
- **Result.** Load tests all PASS. `animals_compose_transfer` headline `lora_step_100000.pt` is intact: 420 LoRA keys, rank-8, `step=100000`, `epoch=2000`. **Structural note that supersedes OPS-02's flat-420-key contract for this run:** the tensors sit under a `lora_state` sub-dict alongside optimizer and scaler state, so a loader written against the flat layout fails on it. `conditioning_window{,_lora}` hold no `.pt` files (figures only). `residual_diagnostics/delta_structure_unguided/tensors.pt` holds `delta` / `eps_poe` / `eps_mono`, the same quantity the does-the-correction-cause-composition scope calls r_t.
- **Decision.** **`rung1`–`rung4` stays for the five-tier ladder only.** Three buckets added: `artifacts/diagnostics/` (`residual_diagnostics`, `conditioning_window{,_lora}`, `group_a_failure`), `artifacts/scopes/<scope>/` (`animals_compose_transfer`, `compose_scorer`, `poe`), and `artifacts/_shared/presentation/`. All eight **keep**; `group_a_failure` keeps as a reference negative (7G, revisit once the negative claims are written). Dry-run: 8 MOVE+LINK, 32 REFUSE (already filed), 3 SKIP (quarantined). The move itself is not yet applied.
