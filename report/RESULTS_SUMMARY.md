# Results summary: LoRA-Fixes-PoE, stage by stage

**Generated**: 2026-07-22 (progress-brief over plans/01-05 + inventory/ + W&B).
**One screen per stage**: current status, what it produced, its W&B runs (worked/crashed), and copy-paste commands to walk the results.
...
2. **Objective number ≠ `artifacts/rungN` folder.** Objective 3 (Cross-Pair) has no folder of its own: it reuses the LoRA from stage 2. So `artifacts/rung3-group-wise/` = Objective 4, `artifacts/rung4-scale/` = Objective 5.
...
## Stage 1: Overfit (Objective 1) ⚠️ partially landed

**Status**: The first result landed on cat×dog seed 42 (λ=0 byte-identical to PoE, λ=1 two distinct animals by ~ep600). One more group trained (G4 typewriter×cactus, MDS panels owed). G1–G3 single-seed LoRAs still owed (empty 0-byte stubs). Negative controls all failed as intended, which is what makes the LoRA result mean something. The trained run stops climbing at ~40% of the PoE→Mono distance, so what limits it first is how much correction it delivers rather than whether the correction travels to new pairs.
...
- Negative controls: `outputs/group_a_failure/{latent_cnn,latent_unet,frozen_feature_mlp}`, all of which fail (reported).

**W&B**: `wag4z592` (typewriter single-seed overfit) worked. cat×dog overfit was a local run. Do not confuse `wag4z592` with `xcp40234`, which is the typewriter *cross-seed* run from stage 2.
...
## Stage 2: Survive-Noise (Objective 2) ◑ G6 trained, verdict pending
...
> Held-out means kept out of training and used only for testing. Seeds 9–12 were never trained on, so composing on them is what would show the LoRA generalises.
...
- Repo also holds for cat×dog: `taskB__k01_pick1_ep1600__wandb-hbpotmnk` and `taskC__s9_ep1600__wandb-d5b2706v`, from a different and earlier set of runs.
...
## Stage 3: Cross-Pair (Objective 3) ⚠️ code ready, not run

**Status**: Sampler + eval-cache code landed (Plan 12: `sample_heldout --heldout-pair`, `build_eval_cache.py`). **No transfer run executed, no artifacts produced.** Source LoRA is the G6 pool (`pueuo7bl`). This stage only proves the wiring: one pair to its sibling is confounded, because a cat×dog-only LoRA saw no variety, so a hit cannot be told apart from a memorised correction that happens to fit. The transfer test a reviewer would believe is stage 4.
...
## Stage 4: Group-Wise (Objective 4, `artifacts/rung3-group-wise/`) ◑ one short G6 run only

**Status**: The G6 within-group run went through (`in_in`+`out_in`, 43 of 43 runs at a mid-training checkpoint). **It proves the wiring and is not a verdict** (no final-checkpoint evaluation over the trained-against-tested grid, no Task D). G1–G4 not started; ~520 cache runs missing. Held-out siblings must share no concept with the pairs trained on (wolf×husky), since lion×dog shares "dog" and is close to a freebie.

**Produced**
- `REPO/rung3-group-wise/cross_pair/within_group/g6/main__wandb-ow1jo0xq/eval_crossbar/step_020000/`: 43 runs + baseline (45 PNGs). 402M.
- `within_group/{g1,g2,g3,g4}/` dirs exist but are scaffolds (not run).

**W&B** (project `poe-repair-cross-pair`): `ow1jo0xq`, G6 within-group, **worked as a wiring test**.
...
## Stage 5: Scale (Objective 5, `artifacts/rung4-scale/`) ⏸ trained, grid evaluation never run

**Status**: `all_groups` LoRA trained to step 30000 on 40 runs (5 pairs × 8 seeds). **The 2×2 grid of trained-on against unseen was never sampled**: `cells.jsonl` is absent everywhere, so the `out_out` quadrant (pair and seed both unseen, the headline the paper needs) has no data. A second run died early.
...
| Stage | Objective | State | Headline evidence | Owed |
|---|---|---|---|---|
...
| 4 Group-Wise | 4 | ◑ | G6 wiring run, 43 runs (`ow1jo0xq`) | final grid evaluation + Task D; G1–G4 |
| 5 Scale | 5 | ⏸ | `all_groups` 30k (`2em6frqv`) | the grid evaluation and `out_out` (`cells.jsonl` absent) |

**One line for the supervisor**: Stage 1 is real, stage 2's headline pool is trained and passes its training verdict but the held-out-seed proof is still rendering, and stages 3 to 5 are code-ready or partially-trained but have not produced their deciding artifact. What binds is delivery (the ~40% ceiling) rather than transfer.
